...
`.buildchain/minimal-consumer.toml` path, which must remain byte-identical to the
canonical TOML. This path is already present in the protected source used to
bootstrap the transition. The first cutover keeps the published compiler's
`feature/*` task route. After publishing the compiler that supports the other
normal task prefixes, enable `fix/*`, `chore/*`, `docs/*`, `ci/*`, and `refactor/*`.
