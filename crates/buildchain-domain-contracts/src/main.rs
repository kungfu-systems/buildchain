...
use buildchain_domain_contracts::{
...
fn host_response(
    ...
    HostResponse {
        ...
        host: HostIdentity {
            ...
            implementation: "buildchain-domain-contracts-shadow-host",
...
fn run_provider_readback_fixtures(fixture_path: &str) -> Result<(), String> {
    let bytes = fs::read(fixture_path).map_err(|error| format!("cannot read fixtures: {error}"))?;
    let projection = run_provider_readback_fixture(&bytes)
        .map_err(|fault| format!("{} at {}: {}", fault.code, fault.path, fault.message))?;
    serde_json::to_writer(std::io::stdout().lock(), &projection)
        .map_err(|error| error.to_string())?;
    println!();
...
fn run_release_activation_request(request_path: &str) -> Result<(), String> {
    let bytes = if request_path == "-" {
        read_stdin()?
    } else {
        fs::read(request_path)
            .map_err(|error| format!("cannot read release activation request: {error}"))?
    };
    let projection = project_release_activation_bytes(&bytes)
        .map_err(|fault| format!("{} at {}: {}", fault.code, fault.path, fault.message))?;
    serde_json::to_writer(std::io::stdout().lock(), &projection)
        .map_err(|error| error.to_string())?;
    println!();
...
fn run_partial_mutation_recovery_request(request_path: &str) -> Result<(), String> {
    let bytes = if request_path == "-" {
        read_stdin()?
    } else {
        fs::read(request_path).map_err(|error| format!("cannot read recovery request: {error}"))?
    };
    let plan = plan_partial_mutation_recovery_bytes(&bytes)
        .map_err(|fault| format!("{} at {}: {}", fault.code, fault.path, fault.message))?;
    serde_json::to_writer(std::io::stdout().lock(), &plan).map_err(|error| error.to_string())?;
    println!();
    Ok(())
            ...
            "delivery-warrant.trace-project" => run_delivery_warrant_trace_fixture(&input)
                .and_then(|value| {
                    serde_json::to_value(value).map_err(|error| {
                        Box::new(buildchain_domain_contracts::ContractFault {
                            schema: buildchain_domain_contracts::CONTRACT_FAULT_CONTRACT.to_owned(),
...
fn run() -> Result<(), String> {
    ...
    match args.as_slice() {
        ...
        [command, fixture_path] if command == "provider-readback" => {
            run_provider_readback_fixtures(fixture_path)
        }
        ...
        _ => Err("usage: buildchain-domain-contracts [trace|compatibility-facts|stage-capsule|stage-capsule-store|provider-operation-journal|provider-readback|resume-plan|release-activation|stable-publication|partial-mutation-recovery|adopter-delivery-parity INPUT.json|host]".to_owned()),
    }
}

fn main() {
    if let Err(error) = run() {
        eprintln!("buildchain-domain-contracts: {error}");
        std::process::exit(65);
    }
}
