...
#[unsafe(no_mangle)]
pub extern "C" fn buildchain_domain_wasm_abi_version() -> u32 {
    ABI_VERSION
}

#[unsafe(no_mangle)]
pub extern "C" fn buildchain_domain_wasm_alloc(length: usize) -> *mut u8 {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn buildchain_domain_wasm_dealloc(pointer: *mut u8, length: usize) {
    if length == 0 {
        return;
    }
    // SAFETY: callers may release only buffers returned by buildchain_domain_wasm_alloc
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn buildchain_domain_wasm_invoke(pointer: *const u8, length: usize) {
    // SAFETY: the Node host writes exactly `length` initialized bytes into a
    // buffer returned by buildchain_domain_wasm_alloc before invoking this export.
...
#[unsafe(no_mangle)]
pub extern "C" fn buildchain_domain_wasm_response_pointer() -> *const u8 {
    RESPONSE.with(|slot| slot.borrow().as_ptr())
}

#[unsafe(no_mangle)]
pub extern "C" fn buildchain_domain_wasm_response_length() -> usize {
