...
struct VersionProjection {
    ...
    version: String,
    #[serde(default)]
    completed_stable_version: Option<String>,
}

pub fn validate_source_version_projection(payload: &Value) -> ContractResult<Value> {
    let input: VersionProjection = parse(payload)?;
    if let Some(stable) = &input.completed_stable_version {
        let pattern = regex::Regex::new(r"^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$")
            .map_err(|error| fault(error.to_string()))?;
        let matched = pattern
            .captures(stable)
            .ok_or_else(|| fault("completed stable version is invalid"))?;
        let patch = matched[3]
            .parse::<u64>()
            .ok()
            .and_then(|value| value.checked_add(1))
            .ok_or_else(|| fault("patch overflow"))?;
        let alpha = regex::Regex::new(
            r"^(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)-alpha\.(0|[1-9][0-9]*)$",
        )
        .map_err(|error| fault(error.to_string()))?;
        if (input.base_version != *stable
            && (!alpha.is_match(&input.base_version)
                || input.base_version.split("-alpha.").next() != Some(stable.as_str())))
            || input.version != format!("{}.{}.{}-alpha.0", &matched[1], &matched[2], patch)
        {
            return Err(fault(
                "only the completed stable patch's next alpha zero is a bounded projection",
            ));
        }
        return Ok(json!({ "valid": true, "validationKind": "version-state-projection" }));
    }
    ...
    let matched = pattern
        ...
        .ok_or_else(|| fault("base is not an alpha version"))?;
