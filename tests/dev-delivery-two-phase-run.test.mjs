...
test("GitHub wake preserves a production-shape exact candidate under one dispatch property", async () => {
  const requests = [];
  const client = new GitHubTwoPhaseClient({
    repository: "kungfu-systems/buildchain",
    token: "fixture",
  ...
  const candidate = {
    ...
    environmentRoot: ROOT("0"),
    affectedPaths: ["packages/native"],
    shardEvidenceRoots: [ROOT("b")],
    nativeCommandContract: NATIVE_COMMAND_CONTRACT,
    releaseBlockerPriority: {
      schema: "kungfu.buildchain.release-blocker-priority-claim/v1",
      claimRoot: ROOT("c"),
    },
...
test("two-phase controller reuses disjoint native proof and qualifies before merge admission", async () => {
  ...
  const proof = createNativeQualificationProof({
    ...
    qualifiedAt: "2026-08-11T00:00:31Z",
  });
  const proofPath = path.join(directory, "proof.json");
  fs.writeFileSync(proofPath, `${JSON.stringify(proof)}\n`);
  const selectedWarrant = warrant();
  fs.writeFileSync(
    path.join(directory, "warrant.json"),
    `${JSON.stringify({ receiptRoot: ROOT("c"), observation: { activeWarrant: selectedWarrant } })}\n`,
  );
  const calls = [];
  try {
    const result = await runTwoPhaseDelivery(options(directory, proofPath), {
      ...
      runCommand: async (input) => {
        ...
        return {
          receiptRoot: ROOT("a"),
          observation: {
            activeWarrant: { ...selectedWarrant, phase: "qualified" },
          },
        };
      },
    });
...
test("duplicate controller dispatch replays the same qualified Warrant without side effects", async () => {
  ...
  const qualifiedWarrant = warrant({
    phase: "qualified",
    nativeProofRoot: ROOT("a"),
    nativeProofReuseRoot: ROOT("b"),
    qualificationReceiptRoot: ROOT("c"),
  });
  fs.writeFileSync(
    path.join(directory, "warrant.json"),
    `${JSON.stringify({ receiptRoot: ROOT("d"), observation: { activeWarrant: qualifiedWarrant } })}\n`,
  );
  let commandCalls = 0;
  try {
    ...
    assert.equal(result.nativeProofRoot, qualifiedWarrant.nativeProofRoot);
    assert.equal(
      result.nativeReuseDecisionRoot,
      qualifiedWarrant.nativeProofReuseRoot,
    );
    assert.equal(result.qualificationReceiptRoot, ROOT("c"));
...
test("controller reclassifies a completed proof after concurrent Dev movement", async () => {
  ...
  fs.writeFileSync(
    path.join(directory, "warrant.json"),
    `${JSON.stringify({ observation: { activeWarrant: selectedWarrant } })}\n`,
  );
  ...
  try {
    await assert.rejects(
      runTwoPhaseDelivery(configured, {
        ...
        composeCandidate: () => {},
        runNative: async () => {
          throw new Error("native proof cannot be reused");
        },
        runCommand: async (input) => {
          ...
          return {
            receiptRoot: ROOT("a"),
            receipt: { successorWake },
            observation: {
              queued: [{ position: 1, candidateId: ROOT("b") }],
            },
          };
        },
      }),
    ...
    assert.equal(wakeCalls[0][1].environmentRoot, ROOT("0"));
    assert.equal(wakeCalls[0][1].sourceProofRoot, ROOT("5"));
    assert.equal(
      JSON.parse(
...
test("unproven fenced-worker termination retains the provisional Warrant", async () => {
  const directory = fs.mkdtempSync(
    path.join(os.tmpdir(), "buildchain-two-phase-unproven-stop-"),
  );
  const configured = {
    ...options(directory, ""),
  ...
  try {
    await assert.rejects(
      runTwoPhaseDelivery(configured, {
        ...
        runCommand: async (input) => {
          commandCalls.push(input);
          throw new Error("must not close an unproven worker");
        },
      }),
      /native worker did not stop after fencing loss/u,
    );
    assert.deepEqual(commandCalls, []);
    const retained = JSON.parse(
      fs.readFileSync(
        path.join(directory, "failure-warrant-retained.json"),
        "utf8",
      ),
    );
    assert.equal(retained.workerTerminationProven, false);
    assert.equal(retained.fencingToken, selectedWarrant.fencingToken);
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
});
