...
function fixture() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "product-provider-"));
  fs.writeFileSync(
  ...
  const payload = {
    ...
    files: [npm, asset].sort((left, right) => left.path.localeCompare(right.path)),
  };
  const candidate = {
    ...payload,
  ...
  fs.writeFileSync(requiredArtifactsPath, `${JSON.stringify(requiredArtifacts, null, 2)}\n`);
  return {
    cwd,
    manifest,
    manifestPath,
...
function githubProvider() {
  ...
  const commits = new Map([[SOURCE, { sha: SOURCE, tree: { sha: "source-tree" }, parents: [] }]]);
  const pullRequests = [];
  const checks = [];
  ...
  const rest = {
    git: {
      ...
      async createRef({ ref, sha }) {
        ...
        if (refs.has(normalized)) throw Object.assign(new Error("exists"), { status: 422 });
...
function npmProvider(cwd, { initialIntegrity = "", visibilityLagReads = 0 } = {}) {
  ...
  return {
    ...
    spawn(command, args) {
      ...
      if (args[0] === "pack") {
        ...
        fs.writeFileSync(path.join(destination, filename), "rematerialized bytes");
        return {
          status: 0,
...
export { SOURCE, VERSION_STATE, REBASED_VERSION_STATE, digest, fixture, githubProvider, npmProvider, productScenario };
