...
import {
  ADOPTER_DELIVERY_ARCHIVE_AUTHORITY,
  ADOPTER_DELIVERY_SOURCE,
...
test("public schema, config and source authority bind the exact v3 and v4 cuts", () => {
  ...
  assert.deepEqual(ADOPTER_DELIVERY_SOURCE, {
    branch: "dev/v3/v3.0",
...
test("exact N-1 bootstrap lineage passes and substitutions fail closed", () => {
  const result = qualifyAdopterDeliveryBootstrap(bootstrap);
  assert.equal(
  ...
  for (const mutate of [
    ...
    mutate(changed);
    assert.throws(
      () => qualifyAdopterDeliveryBootstrap(changed),
...
test("published archives load from exact bytes and v4 rejects non-authority identities", async (t) => {
  ...
  assert.deepEqual(
    assertPublishedAdopterDeliveryRequest(officialRequest),
    officialRequest,
  );
  assert.deepEqual(ADOPTER_DELIVERY_ARCHIVE_AUTHORITY, {
    buildchain: {
      name: "@kungfu-tech/buildchain",
      version: "3.0.9-alpha.16",
      archiveRoot:
