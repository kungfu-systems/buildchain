...
function qualification(overrides = {}) {
  ...
  return {
    ...body,
    reportRoot: domainContentRoot("semantic-diff", body),
  };
}
...
function hostResponse(request, state, overrides = {}) {
  return {
    ...
    structuredResult: {
      ...
      state,
      stateRoot: domainContentRoot("queue-state", state),
    },
    ...overrides,
  };
}
...
test("explicit v4 read returns the legacy observation after an effect-disabled Rust projection", async () => {
  const retained = [];
  const result = await runDeliveryWarrantReadCandidate(
    queue(),
    candidateOptions({
      retain(evidence) {
...
test("caller switch defaults to v3 and affects observe only", async () => {
  ...
  const domainObserved = await runDevDeliveryCommand(
    {
      command: "observe",
      repository: "kungfu-systems/kungfu",
      branch: "dev/v4/v4.0",
  ...
  assert.equal(domainObserved.readMode, "v4");
  assert.equal(domainObserved.readCandidate.rustAuthority, "read-only");
...
test("the real Rust host validates and projects the candidate state without effects", async () => {
  ...
  assert.deepEqual(
    ...
    projectBaselineQueueToReadState(queue()),
  );
  assert.equal(result.rustEffects, "disabled");
});
