...
test("signing and attestation retain isolated credentials and final-byte checks", () => {
  ...
  const action = readComposite("build/sign-artifact");
  ...
  assertOrder(readRepoText("packages/core/build/commands/stage.mjs"), ["runLifecycle(lifecycleOptions", "await sealSigning", "seal-macos-credential-input.mjs"]);
  const signing = readRepoText("packages/core/build/commands/sign.mjs");
  ...
  assert.match(readRepoText("packages/core/build/commands/attest.mjs"), /--deny-self-hosted-runners/u);
});
...
test("infrastructure configuration has one governed authority", () => {
  ...
  assert.match(readRepoText("packages/core/build/commands/stage.mjs"), /SHIFU_CACHE_PROFILE_DIGEST/u);
  assert.match(readRepoText("packages/core/build/commands/prepare.mjs"), /BUILDCHAIN_CHECKOUT_CACHE_FETCH_ATTEMPTS/u);
});
