...
test("v4 tail-reseal fixture and planner bind the exact four-platform retained candidate", () => {
  ...
  const normalized = normalizeTailResealRequest(fixture);
  const first = planTailReseal(normalized);
  const second = planTailReseal(structuredClone(fixture));
  assert.deepEqual(
...
test("tail receipt permits only the explicit macOS byte change and embeds the standard v4 candidate Passport", () => {
  ...
  assert.equal(
    verifyTailResealReceipt({ receipt, request: fixture, passport }).ok,
    true,
  );
...
test("fresh-runner byte verification proves retained bytes and only accepts a read-back macOS reseal", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "buildchain-tail-bytes-"));
  ...
  const before = verifyTailResealPlatform({
    request,
  ...
  const after = verifyTailResealPlatform({
    request,
    platformId: "macos-arm64",
    artifactRoot: root,
    mode: "resealed",
...
test("GitHub admission readback binds archive digests and the single prior macOS failure", () => {
  const evidence = {
    ...
    sourceJobs: [
      ...TAIL_RESEAL_REQUIRED_SUCCESS_JOBS.map((name, index) => ({
        id: index + 1,
        name,
        conclusion: "success",
        steps: [],
  ...
  const admission = validateTailResealGitHubEvidence(evidence);
  ...
  assert.throws(
    () => validateTailResealGitHubEvidence(stale),
  ...
  assert.throws(
    () => validateTailResealGitHubEvidence(extraFailure),
