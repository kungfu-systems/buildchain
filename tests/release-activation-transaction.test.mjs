...
test("activation fails closed on skipped phase and changed replay roots", () => {
  ...
  assert.throws(
    () => recordReleaseActivationPhase(transaction, "site-published", { receiptRoots: [root("a")] }),
    /cannot skip/,
  );
  const qualified = recordReleaseActivationPhase(transaction, "candidate-qualified", {
    receiptRoots: [root("2")],
  });
  assert.throws(
    () => recordReleaseActivationPhase(qualified, "candidate-qualified", { receiptRoots: [root("3")] }),
    /replay changed/,
  );
});
...
test("receipt synthesis requires five exact receipt bindings", () => {
  ...
  assert.equal(validateReleaseActivationReceiptSet(receiptSet, { allowShadow: false }).valid, false);
  assert.throws(
    () => createReleaseActivationReceiptSet({
      transaction,
...
test("partial failure, retry, abort, and rollback remain explicit", () => {
  ...
  transaction = recordReleaseActivationPhase(transaction, "candidate-qualified", {
    receiptRoots: [root("2")],
  });
  transaction = recordReleaseActivationPhase(transaction, "artifacts-published", {
  ...
  const aborted = abortReleaseActivationTransaction(transaction, "operator stopped shadow");
