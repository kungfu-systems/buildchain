...
test("JavaScript and Rust adapters emit one semantic projection for retained traces", () => {
  for (const name of ["golden.json", "replay.json"]) {
    ...
    const javascript = runDeliveryWarrantTraceFixture(bytes);
...
test("semantic projection retains faults, ordered effects, observations, and receipts", () => {
  const golden = runDeliveryWarrantTraceFixture(readFixture("golden.json"));
  assert.deepEqual(
  ...
  const replay = runDeliveryWarrantTraceFixture(readFixture("replay.json"));
  assert.equal(
...
test("both adapters fail closed on malformed, incomplete, reordered, stale-root, and unsupported traces", () => {
  ...
  for (const [code, bytes] of cases) {
    assert.throws(
      () => runDeliveryWarrantTraceFixture(bytes),
...
test("the trace schema and pure runners remain closed and provider-free", () => {
  ...
  const rust = fs.readFileSync(
    new URL(
      "../crates/buildchain-domain-contracts/src/trace.rs",
      import.meta.url,
    ),
    "utf8",
  );
...
test("TypeScript shadow passes identical bytes to legacy and Rust while retaining no second authority", async () => {
  ...
  const seen = [];
  const retained = [];
  const result = await runDeliveryWarrantShadow(bytes, {
    enabled: true,
    ...
    invokeLegacy(input) {
      ...
      return runDeliveryWarrantTraceFixture(input);
    },
    invokeRust(request) {
      const input = Buffer.from(request.input.bytes, "base64");
      seen.push(input);
      return hostResponse(request, runDeliveryWarrantTraceFixture(input));
    },
    retain(observation) {
...
test("the default Rust shadow host emits an effect-disabled projection", async () => {
  const result = await runDeliveryWarrantShadow(readFixture("replay.json"), {
    enabled: true,
    recordedAt: "2026-08-08T00:00:00.000Z",
    sources,
    timeoutMs: 30_000,
...
test("disabled, unsafe, and invalid shadow configuration never changes the legacy result", async () => {
  ...
  const unsafe = await runDeliveryWarrantShadow(
...
test("timeout, crash, cancellation, malformed response, unsupported host, and retention failure are bounded observations", async () => {
  ...
  const unsupported = await runDeliveryWarrantShadow(bytes, {
    enabled: true,
    recordedAt: "2026-08-08T00:00:00.000Z",
    sources,
    invokeRust(request) {
  ...
  const cancelled = await runDeliveryWarrantShadow(bytes, {
    enabled: true,
    recordedAt: "2026-08-08T00:00:00.000Z",
    sources,
  ...
  const retentionFailure = await runDeliveryWarrantShadow(bytes, {
    enabled: true,
    recordedAt: "2026-08-08T00:00:00.000Z",
    sources,
    invokeRust(request) {
      return hostResponse(request, runDeliveryWarrantTraceFixture(bytes));
    },
    retain() {
...
const semanticOptions = {
  ...
  retainObservation() {},
  retain(report) {
    return { receiptRoot: domainContentRoot("semantic-diff", report) };
  },
};
...
test("semantic diff gate binds complete zero-diff coverage and detects bounded faults", async () => {
  ...
  const report = await runDeliveryWarrantSemanticDiffGate(
    semanticCases,
    semanticOptions,
  );
...
test("semantic diff gate retains an explainable blocker for every unexplained change", async () => {
  const report = await runDeliveryWarrantSemanticDiffGate(semanticCases, {
    ...semanticOptions,
    runShadow(value, options) {
      return runDeliveryWarrantShadow(value, {
        ...options,
        invokeRust(request) {
          const projection = runDeliveryWarrantTraceFixture(
  ...
  const unretained = await runDeliveryWarrantSemanticDiffGate(semanticCases, {
    ...semanticOptions,
