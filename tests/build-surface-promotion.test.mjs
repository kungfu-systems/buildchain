...
test("buildchain ref promotion consumes PR-stage release candidate evidence", () => {
  ...
  assert.match(workflow, /promote-stable:[\s\S]*uses: kungfu-systems\/buildchain\/\.github\/workflows\/release-candidate-promote\.yml@v4/); assert.doesNotMatch(workflow, /promote-alpha:[\s\S]*uses: kungfu-systems\/buildchain\/\.github\/workflows\/release-candidate-promote\.yml@alpha\/v4\/v4\.0/);
  ...
  assert.match(
    workflow,
    /promote-alpha:[\s\S]*buildchain-ref: \$\{\{ inputs\['resume-candidate-run-id'\] != '' && needs\.alpha-recovery-admission\.outputs\.runtime-sha \|\| 'v4-alpha' \}\}/,
  );
...
test("Buildchain self-dogfoods the current major through floating public promotion", () => {
  ...
  assert.match(promotion, /buildchain-ref: \$\{\{ inputs\['resume-candidate-run-id'\] != '' && needs\.alpha-recovery-admission\.outputs\.runtime-sha \|\| 'v4-alpha' \}\}/);
