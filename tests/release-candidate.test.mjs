...
test("release candidate resolver traces a major gate through its exact release-line merge", async () => {
  ...
  const releaseHeadSha = "9".repeat(40);
  ...
  const fetchImpl = async (url) => {
    if (url.endsWith(`/commits/${majorSha}/pulls`)) {
      return jsonResponse([
        {
          number: 1711,
          state: "closed",
    ...
    if (url.endsWith(`/commits/${releaseSha}/pulls`)) {
      return jsonResponse([
        {
          number: 1710,
          state: "closed",
          merged_at: "2026-07-24T21:08:35Z",
          merge_commit_sha: releaseSha,
          base: { ref: "release/v2/v2.14" },
          head: {
            ref: "fix/release-line-v2-v2.14-refresh-major-approver",
            sha: releaseHeadSha,
            repo: { full_name: "kungfu-systems/buildchain" },
          },
        },
        ...
        workflow_runs: [
          {
            ...
            status: "completed",
            conclusion: "success",
            head_sha: releaseHeadSha,
            pull_requests: [{ number: 1710 }],
          },
        ],
      });
    }
    if (url.includes("actions/runs/30126503376/artifacts")) {
      return jsonResponse({
        artifacts: [
          { id: 1, name: `libnode-shaped-release-candidate-${builtSourceSha}`, expired: false },
          { id: 2, name: `libnode-shaped-summary-${builtSourceSha}`, expired: false },
  ...
  assert.equal(result.pullRequest.number, 1710);
  assert.equal(result.promotionPullRequest.number, 1711);
  assert.equal(result.run.id, "30126503376");
  assert.equal(result.artifacts.sourceSha, builtSourceSha);
});

test("release candidate resolver rejects a major gate whose release head is not the exact release PR merge", async () => {
  const majorSha = "b".repeat(40);
...
test("release candidate resolver deduplicates byte-identical explicit GitHub Release assets", () => {
  const workspace = fs.mkdtempSync(path.join(os.tmpdir(), "buildchain-rc-release-assets-"));
  try {
    fs.mkdirSync(path.join(workspace, "linux"), { recursive: true });
    fs.mkdirSync(path.join(workspace, "macos", "signed"), { recursive: true });
    ...
    fs.writeFileSync(path.join(workspace, "macos", "signed", "kungfu-macos.tar.gz"), "b");
    ...
    assert.deepEqual(
      ...
      [path.join(workspace, "linux", "kungfu-linux.tar.gz"), path.join(workspace, "macos", "kungfu-macos.tar.gz")],
    );
    assert.throws(
      () =>
        selectReleaseAssetPaths({
