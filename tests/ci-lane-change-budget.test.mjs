...
function declaration(overrides = {}) {
  return {
    laneId: ".github/workflows/.build-check.yml#required",
...
test("legacy baseline lanes do not require retroactive declarations", () => {
  const result = evaluateCiLaneChangeBudget({
    policy: policy({
      baseline: { lanes: [".github/workflows/.build-check.yml#required"] },
    }),
    workflows: [{ path: ".github/workflows/.build-check.yml", text: workflow }],
  });
...
test("a complete exact-lane declaration admits the new lane", () => {
  ...
  assert.equal(result.ok, true);
  assert.deepEqual(result.newLanes, [".github/workflows/.build-check.yml#required"]);
});
