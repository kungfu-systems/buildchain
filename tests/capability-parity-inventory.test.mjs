...
import {
  BASELINE_CAPABILITY_CUTS,
  assertBaselineCapabilityInventory,
  buildBaselineCapabilityInventory,
} from "../scripts/check-capability-parity-inventory.mjs";
import {
  assertCapabilityCutAncestor,
  ensureCapabilityCutAncestor,
  ensureCapabilityCutLineage,
} from "../scripts/capability-parity-catalog.mjs";
...
test("live v4 cut has an admitted lineage from the checked candidate", () => {
  ...
  assert.throws(
    () =>
      assertCapabilityCutAncestor({
        revision: "HEAD",
        descendant: BASELINE_CAPABILITY_CUTS.liveV4,
...
test("live v3 inventory covers every declared category with no unknown or unowned capability", () => {
  const inventory = buildBaselineCapabilityInventory({ root: process.cwd() });
  assert.equal(
...
test("checked-in inventory is structurally valid", () => {
  const inventory = JSON.parse(
    fs.readFileSync(
      "architecture/capability-parity-live-capability-inventory.json",
      "utf8",
    ),
  );
  assert.doesNotThrow(() => assertBaselineCapabilityInventory(inventory));
});
...
test("unknown, unowned, and nonexistent-route mutations fail closed", () => {
  ...
  assert.throws(
    () => assertBaselineCapabilityInventory(unknown),
  ...
  assert.throws(
    () => assertBaselineCapabilityInventory(unowned),
  ...
  route.v4Route = null;
  assert.throws(
    () => assertBaselineCapabilityInventory(routed),
  ...
  assert.throws(
    () => assertBaselineCapabilityInventory(substituted),
