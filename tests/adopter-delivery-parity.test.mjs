...
const parityPlan = JSON.parse(
  fs.readFileSync(
    new URL("../architecture/adopter-delivery-parity.json", import.meta.url),
    "utf8",
  ),
);
...
function project(input) {
  const projection = assertAdopterDeliveryParity(input);
  assert.equal(
    projection.schema,
    ADOPTER_DELIVERY_PARITY_PROJECTION_CONTRACT,
  );
...
test("v4 parity plan binds the protected v3 vector suite and excludes writer authority", () => {
  assert.deepEqual(parityPlan.sourceAuthority, {
    branch: "dev/v3/v3.0",
    commit: ADOPTER_DELIVERY_PARITY_SOURCE.v3Commit,
    vectorSuiteRoot: ADOPTER_DELIVERY_PARITY_SOURCE.vectorSuiteRoot,
    kfdPackage: ADOPTER_DELIVERY_PARITY_SOURCE.kfdPackage,
  });
...
test("effect-disabled v4 core reproduces two isolated drivers and offline replay", () => {
  ...
  const alpha = createAdopterDeliveryParityInput({
    vectorId: "golden-two-driver-offline-replay",
    request: request(),
    drivers,
    artifactProfiles,
  });
  const beta = createAdopterDeliveryParityInput({
    vectorId: "golden-two-driver-offline-replay",
...
test("published KFD category conflict and stale package decisions match v3 exactly", () => {
  ...
  const conflictInput = createAdopterDeliveryParityInput({
  ...
  const staleInput = createAdopterDeliveryParityInput({
...
test("substituted expected decisions and authority roots fail closed", () => {
  const input = createAdopterDeliveryParityInput({
    vectorId: "negative-driver-mismatch",
    request: request("example.protocol/unknown"),
    drivers: [driver("example.protocol/alpha")],
  ...
  assert.throws(
    () => runAdopterDeliveryParity(substitutedDecision),
  ...
  assert.throws(
    () => runAdopterDeliveryParity(substitutedAuthority),
