...
function copyAuthority(directory, { tamper = false, omit = false } = {}) {
  ...
  for (const name of ["domain-wasm.js", "domain-wasm-artifact.js"]) {
    fs.copyFileSync(path.join(coreDirectory, name), path.join(directory, name));
  }
  if (!omit) {
    const bytes = fs.readFileSync(artifactPath);
    if (tamper) bytes[bytes.length - 1] ^= 1;
    fs.writeFileSync(path.join(directory, "buildchain-domain.wasm"), bytes);
  }
}

async function assertAuthorityUnavailable(directory) {
  const module = await import(
    `${pathToFileURL(path.join(directory, "domain-wasm.js")).href}?case=${crypto.randomUUID()}`
  );
  assert.throws(
    () => module.domainWasmInfo(),
...
test("tracked Rust/WASM artifact is bound to its ABI metadata", () => {
  const bytes = fs.readFileSync(artifactPath);
  assert.equal(sha256(bytes), DOMAIN_WASM_SHA256);
  ...
  assert.deepEqual(domainWasmInfo(), {
    abiVersion: DOMAIN_WASM_ABI_VERSION,
    requestContract: DOMAIN_WASM_REQUEST_CONTRACT,
    responseContract: DOMAIN_WASM_RESPONSE_CONTRACT,
  });
});
...
test("a clean Node process loads the committed artifact without Rust", () => {
  ...
  assert.deepEqual(JSON.parse(result.stdout), domainWasmInfo());
});
