...
test("Stage Capsule freezes byte-identical identity, capsule, and availability roots", () => {
  ...
  assert.equal(
    stageCapsuleIdentityRoot(entry.capsule.identity),
    entry.capsule.identityRoot,
  );
  assert.equal(stageCapsuleRoot(entry.capsule), entry.capsule.capsuleRoot);
  assert.equal(validateStageCapsule(entry.capsule), entry.capsule);
  const availabilityRoot = stageCapsuleAvailabilityRoot(entry.availability);
  assert.equal(
    availabilityRoot,
    "sha256:a9656d2944a4f98b474b0fecc339f6116d6b7f966cf3186dca906ea9e64168bd",
  );
  assert.deepEqual(evaluateStageCapsuleReuse(requestFor(entry)), {
    schema: "buildchain-v4-stage-capsule-reuse/v1",
    eligible: true,
...
test("semantic identity invalidates exactly on platform, stage, policy, transformation, and inputs", () => {
  const entry = fixtures.validCases[0];
  const original = stageCapsuleIdentityRoot(entry.capsule.identity);
  const mutations = [
  ...
  for (const [name, mutate] of mutations) {
    ...
    mutate(identity);
    const changed = stageCapsuleIdentityRoot(identity);
  ...
  transportObservation.transports[0].root =
    "sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd";
  assert.notEqual(
    stageCapsuleAvailabilityRoot(transportObservation),
    stageCapsuleAvailabilityRoot(entry.availability),
  );
  assert.equal(
    stageCapsuleIdentityRoot(entry.capsule.identity),
    original,
...
test("reuse fails closed for missing, expired, corrupt, and root-mismatched content", () => {
  ...
  const unavailable = (status) => {
    ...
    return evaluateStageCapsuleReuse(requestFor(entry, { availability }));
  };
  for (const status of [
    "missing",
    "partial",
...
test("closed contracts exclude provider ids, artifact ids, paths, credentials, and ambient time", () => {
  ...
  const cases = [
    ...
    () => validateStageCapsule({ ...entry.capsule, artifactId: "artifact-7" }),
    () =>
      validateStageCapsuleAvailability({
        ...entry.availability,
  ...
  assert.deepEqual(rust.validCases, [
    {
      ...
      capsuleRoot: entry.capsule.capsuleRoot,
      availabilityRoot: stageCapsuleAvailabilityRoot(entry.availability),
      reuse: evaluateStageCapsuleReuse(requestFor(entry)),
    },
  ]);
...
test("architecture manifest budgets one schema, one writer, and zero provider or v3 authority drift", () => {
  const contract = JSON.parse(
    fs.readFileSync(
      new URL("../architecture/stage-capsule-contract.json", import.meta.url),
      "utf8",
    ),
  );
  assert.equal(
