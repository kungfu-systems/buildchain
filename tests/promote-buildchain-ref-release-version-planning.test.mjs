...
test("release line dry-run explains alpha promotion semantics", () => {
  ...
  const domain = explainReleaseLineDryRun({ cwd, targetRef: "alpha/v4/v4.0", sha: SHA });
  assert.match(domain.branchUpdates[1].action, /next alpha version on dev through a separate protected PR after publication/);
  assert.deepEqual(domain.exactTags.map(({ kind }) => kind), ["alpha"]);
});
...
test("version verification ignores generated buildchain evidence", () => {
  ...
  fs.mkdirSync(path.join(cwd, ".buildchain/runtime/actions/release/promote-ref"), { recursive: true });
  fs.writeFileSync(
    path.join(cwd, ".buildchain/runtime/actions/release/promote-ref/action.yml"),
    "name: runtime\n",
  );
