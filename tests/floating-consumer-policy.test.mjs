...
const policy = JSON.parse(
  fs.readFileSync(
    path.join(root, "architecture/floating-consumer-policy.json"),
    "utf8",
  ),
);
...
function certifyFromExactCaller(result, receipt = result.receipt, receiptRoot) {
  ...
  writeJson(input, {
    receipt,
    receiptRoot: receiptRoot ?? floatingConsumerDocumentRoot(receipt),
  });
...
test("receipt verification and external certification fail closed on stale roots", () => {
  ...
  const verified = verifyFloatingConsumerPolicyReceipt({
  ...
  const certified = certifyFloatingConsumerPolicyReceipt({
    receipt: result.receipt,
    receiptRoot: result.receiptRoot,
...
test("external certification rejects a self-authored certification document", () => {
  ...
  forged.certificationRoot = floatingConsumerDocumentRoot(forged.certification);
...
test("an old Buildchain runtime cannot self-authorize without a rooted receipt", () => {
  const certification = certifyFloatingConsumerPolicyReceipt({
    receipt: undefined,
    receiptRoot: "",
    repository: "kungfu-systems/consumer",
    sourceSha: SOURCE_SHA,
...
test("final v4 Release Passport construction requires fresh external certification", () => {
  ...
  forgedScanner.certificationRoot = floatingConsumerDocumentRoot(
