...
function receipt(requestValue = request()) {
  return {
    ...
    status: "succeeded",
    requestRoot: universalWorkflowRequestRoot(requestValue),
    admissionRoot: root("a"),
...
test("successful Train delivery binds one exact protected backflow PR", () => {
  const plan = createUniversalBackflowPlan({
    request: request(),
    receipt: receipt(),
  });
...
test("failed or non-Train deliveries cannot claim backflow", () => {
  ...
  assert.throws(() =>
    createUniversalBackflowPlan({ request: request(), receipt: failed }),
  );
  const exact = request();
  exact.mode = "exact";
  exact.candidate.discoveryRef = sha("1");
  assert.throws(() =>
    createUniversalBackflowPlan({ request: exact, receipt: receipt(exact) }),
  );
