...
test("historical phase-less dequeue closes instead of inheriting provisional retention", () => {
  ...
  const legacyQueue = structuredClone(selected.queue);
  delete legacyQueue.activeWarrant.phase;
  delete legacyQueue.stateRoot;
  legacyQueue.stateRoot = devDeliveryContentRoot(legacyQueue);
  assert.equal(Object.hasOwn(legacyQueue.activeWarrant, "phase"), false);
  const closed = settleDevDeliveryTerminalEvent(
    legacyQueue,
    {
