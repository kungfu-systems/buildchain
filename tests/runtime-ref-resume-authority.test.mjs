...
function authorize({
  scan = scanRuntimeSelectorPersistence({ root: cleanConsumer() }),
  ...overrides
} = {}) {
  return authorizeRuntimeSelection({
...
test("runtime authorization fails closed on event, permission, reachability, stale roots, and persistence", () => {
  ...
  const failedScan = scanRuntimeSelectorPersistence({
    root: cleanConsumer(),
  });
  failedScan.status = "rejected";
  failedScan.failures = [{ code: "persistent-runtime-default" }];
  failedScan.root = runtimeResumeDocumentRoot(
