...
const CONTRACT_ROOT = nextDevelopmentRoot({ contract: "v4-alpha" });
...
test("fresh runner reuses durable adapter output, handles moved Dev and waits for protected PR", async () => {
  ...
  assert.equal(evidence.models.semverAuto.targetVersion, "4.0.9-alpha.2");
...
test("same-runner recovery and every rooted boundary fail closed", async () => {
  ...
  for (const mutate of [
    (forged) => {
      forged.recovery.transientDurableStateFailures = 0;
    },
    (forged) => {
      forged.adapter.operations[0].materializationRoot = CONTRACT_ROOT;
    },
    (forged) => {
      forged.models.semverAuto.controller.readback.agrees = false;
    },
  ]) {
...
test("hosted evidence roots the exact protected Dev readback and floating refs", async () => {
  ...
  drifted.hostedProtectedDevReadback.versionRoots[0].gitBlobSha = "4".repeat(
    40,
  );
  assert.throws(
...
test("floating v4 production coordinate is mandatory", async () => {
