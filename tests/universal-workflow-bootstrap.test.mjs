...
import {
  UNIVERSAL_WORKFLOW_ADMISSION_POLICY,
...
test("a protected Alpha merge binds reviewed head to the exact runtime", () => {
  ...
  const admit = (candidateRequest = requestValue, candidateEvidence = evidence) => admitUniversalWorkflow({ ...consumerObservation(), request: candidateRequest, policy: policyValue, observedRefSha: sha("1"), reviewEvidence: candidateEvidence, now: "2026-08-30T12:00:00.000Z" }); assert.equal(admit().runtime.sha, sha("1"));
...
test("real universal promotion materializes one rooted product intent before APPLY", () => {
  ...
  assert.equal(selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Fresh" }), "");
  ...
  assert.equal(productStateVersion(state(version).stateRef, sourceSha), version);
  assert.equal(selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", recoveryStates: [state(version, "6")] }), version);
  assert.throws(() => selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", recoveryStates: [state(candidateVersion), state(version, "6")] }), { code: "recovery-state-ambiguous" });
  assert.equal(selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", recoveryStates: [{ ...state(version), exactTagRef: undefined }] }), version);
  assert.equal(selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", explicitResume: true }), candidateVersion);
  assert.equal(selectRecoveredProductPublicationVersion({ ...recovery, candidateVersion: version, routeDecision: "Resume", exactTagRef: { ref: `refs/tags/v${version}`, object: { type: "commit", sha: sourceSha } } }), version);
  assert.throws(() => selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume" }), { code: "recovery-tag-missing" });
  assert.throws(() => selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", recoveryStates: [{ ...state(version), stateCommit: { sha: sha("5"), parents: [{ sha: sha("7") }] } }] }), { code: "recovery-state-mismatch" });
  assert.throws(() => selectRecoveredProductPublicationVersion({ ...recovery, routeDecision: "Resume", recoveryStates: [{ ...state(version), exactTagRef: { ref: `refs/tags/v${version}`, object: { type: "commit", sha: sha("7") } } }] }), { code: "recovery-tag-mismatch" });
});
test("generated product-state finalization heads are semantic no-ops", () => {
  ...
  assert.equal(selectFinalizedProductPublicationVersion({ requestedSha: headSha, requestedTree: treeSha, targetRef: "alpha/v4/v4.0", recoveryStates: [state] }), "4.0.2-alpha.16");
  assert.equal(selectFinalizedProductPublicationVersion({ requestedSha: sourceSha, requestedTree: sha("7"), targetRef: "alpha/v4/v4.0", recoveryStates: [state] }), "");
  assert.throws(() => selectFinalizedProductPublicationVersion({ requestedSha: headSha, requestedTree: treeSha, targetRef: "alpha/v4/v4.0", recoveryStates: [state, { ...state, stateRef: { ref: `refs/heads/buildchain/v4-product-state/${sourceSha}-4-0-2-alpha-17`, object: { type: "commit", sha: sha("8") } }, stateCommit: { sha: sha("8"), tree: { sha: treeSha }, parents: [{ sha: sourceSha }] }, exactTagRef: undefined }] }), { code: "finalization-state-ambiguous" });
});
