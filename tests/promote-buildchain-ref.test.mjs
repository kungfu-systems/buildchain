...
const { explainReleaseLineDryRun, formatReleaseLineDryRun } = await import("../packages/core/release-line-dry-run.js");
const {
...
const { PUBLICATION_ARTIFACT_CANDIDATE_CONTRACT, publicationArtifactCandidateDigest } =
  await import("../packages/core/publication-artifact-candidate.js");
const { createPublicationSealedBundle } = await import("../packages/core/publication-sealed-bundle.js");
const { validateRequiredPublishSourceLock, collectGitHubReleaseEvidenceAssets, publishGitHubReleaseEvidence } =
...
test("release governance preserves the emitted reusable workflow check context", () => {
  ...
  assert.equal(
    resolveProtectedStatusCheckContext({
      ...
      protection: {
        required_status_checks: { strict: true, contexts: ["consumer verify"] },
      },
    }),
    "consumer verify",
  );
...
test("version verification ignores generated buildchain evidence", () => {
  ...
  fs.mkdirSync(path.join(cwd, ".buildchain/release-candidate/passport"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/release-candidate/passport/release-candidate-passport.json"), "{}\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/kfd/kfd-1"), { recursive: true });
  fs.mkdirSync(path.join(cwd, ".buildchain/kfd/kfd-2/claims"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/kfd/kfd-1/contract-world.witness.json"), "{}\n");
  fs.writeFileSync(path.join(cwd, ".buildchain/kfd/kfd-2/claims/buildchain-npm-publish-evidence.json"), "{}\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/release-evidence/v1.0.1"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/release-evidence/v1.0.1/evidence.json"), "{}\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/release-passport"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/release-passport/buildchain.release.json"), "{}\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/release-state"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/release-state/v1.0.1.json"), "{}\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/runtime/actions/promote-buildchain-ref"), { recursive: true });
  fs.writeFileSync(path.join(cwd, ".buildchain/runtime/actions/promote-buildchain-ref/action.yml"), "name: runtime\n");
  fs.mkdirSync(path.join(cwd, ".buildchain/contract-drift"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/contract-drift/issue-body.md"), "# compatible drift\n");
  fs.writeFileSync(path.join(cwd, ".buildchain/publication-result.json"), "{}\n");
  ...
  fs.mkdirSync(path.join(cwd, ".buildchain/admitted/controller"), {
    recursive: true,
  });
  fs.writeFileSync(path.join(cwd, ".buildchain/admitted/controller/receipt.json"), "{}\n");
...
`,
  ...
  const tarballBytes = Buffer.from([0x1f, 0x8b, 0x08, 0x00, 0xff, 0x00, 0x7f, 0x80, 0xfe, 0x42]);
  ...
  const candidatePayload = {
    ...
    files: [fileEntry(tarballPath), fileEntry(assetPath)].sort((left, right) => left.path.localeCompare(right.path)),
  };
  const candidate = {
    ...candidatePayload,
  ...
  const manifest = createPublicationSealedBundle({
    ...
    npmIntegrity: `sha512-${crypto.createHash("sha512").update(tarballBytes).digest("base64")}`,
    releaseAssetPaths: [assetPath],
  });
  const manifestPath = path.join(sourceCwd, ".buildchain/admitted/sealed-bundle.json");
  ...
  const transactionArgs = {
    ...
    publishTransaction: true,
    publishCommand: "node scripts/publish.mjs",
    publishRequiredArtifactsJson: JSON.stringify([{ kind: "npm", name: "@kungfu-tech/paper" }]),
  ...
  const interruptedState = JSON.parse(
    fs.readFileSync(path.join(sourceCwd, `.buildchain/release-state/v${version}.json`), "utf8"),
  );
...
`,
  ...
  const recoveredTarball = path.join(freshCwd, ".buildchain/recovered-publication", version, tarballPath);
  const recoveredAsset = path.join(freshCwd, ".buildchain/recovered-publication", version, assetPath);
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(path.join(freshCwd, "publish-input.json"), "utf8")).tarballPath,
    recoveredTarball,
  );
});
...
test("settled alpha re-enters a complete sealed transaction for GitHub Release resume", async () => {
  const version = "1.0.4-alpha.0";
  const seedCwd = makeTempWorkspace({});
  const relativeTarball = "release/paper-1.0.4-alpha.0.tgz";
  const tarballPath = path.join(seedCwd, relativeTarball);
  const tarballBytes = Buffer.from([0x1f, 0x8b, 0x08, 0x00, 0x7f, 0x80, 0xff]);
  fs.mkdirSync(path.dirname(tarballPath), { recursive: true });
  fs.writeFileSync(tarballPath, tarballBytes);
  const tarballSha256 = crypto.createHash("sha256").update(tarballBytes).digest("hex");
  const candidatePayload = {
    schemaVersion: 1,
    contract: PUBLICATION_ARTIFACT_CANDIDATE_CONTRACT,
    repository: "kungfu-systems/buildchain",
  ...
  const sealedBundle = createPublicationSealedBundle({
    ...
    packageVersion: version,
    npmTarballPath: relativeTarball,
    npmIntegrity: `sha512-${crypto
      .createHash("sha512")
      .update(tarballBytes)
      .digest("base64")}`,
    releaseAssetPaths: [relativeTarball],
    githubReleaseRequired: true,
  });
  const statePath = path.join(
    seedCwd,
    `.buildchain/release-state/v${version}.json`,
  );
  const evidencePath = path.join(
    seedCwd,
    `.buildchain/release-evidence/v${version}/evidence.json`,
  );
  fs.mkdirSync(path.dirname(evidencePath), { recursive: true });
  fs.writeFileSync(
    evidencePath,
  ...
  let transaction = createReleaseTransaction({
    ...
    targetRef: "alpha/v1/v1.0",
    releaseSha: SHA,
    releaseMaterialSha: SHA,
    publishToolingSha: SHA,
    statePath,
    evidencePath,
  });
  ...
  transaction = recordReleaseTransactionMilestone(
    transaction,
    "package-published",
    { artifactCount: 1, sealedBundleRoot: sealedBundle.root },
  );
  ...
  assert.equal(transaction.publication_state, "package-published");
  ...
  await persistDurableReleaseTransaction({
    ...
    extraFiles: [
      {
        path: `${sealedBundle.durablePath}/files/${relativeTarball}`,
...
`,
  ...
  const result = await promoteBuildchainRefs({
    ...
    publishTransaction: true,
    publishRequiredArtifactsJson: JSON.stringify([
      { kind: "npm", name: "@kungfu-tech/buildchain" },
    ]),
  });
  ...
  assert.equal(result.publishTransaction.sealedBundleRoot, sealedBundle.root);
  assert.deepEqual(
    fs.readFileSync(result.publishTransaction.sealedReleaseAssetPaths[0]),
    tarballBytes,
  );
  const completion = await recordGitHubReleaseTransactionCompletion({
    ...
    release: {
      action: "updated",
      tag: `v${version}`,
      url: `https://github.test/releases/tag/v${version}`,
      assetCount: 1,
    },
  });
  assert.equal(completion.transaction.publication_state, "alpha-complete");
  assert.equal(
    completion.transaction.milestones.github_release.tag,
    `v${version}`,
  );
});
...
test("strict alpha promotion protects created dev branches with one required approval", async () => {
  ...
  const octokit = {
    rest: {
      ...
      users: {
        getAuthenticated: async () => ({
          data: { login: "current-release-bot" },
        }),
      },
      apps: {
        getAuthenticated: async () => ({
          data: { slug: "current-release-app" },
        }),
      },
      repos: {
        getBranchProtection: async () => ({
