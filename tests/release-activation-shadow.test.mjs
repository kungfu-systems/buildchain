...
function expectFault(request, code) {
  assert.throws(
    () => projectReleaseActivation(request),
...
test("activation planning is invariant to step, dependency, event, and exact duplicate ordering", () => {
  ...
  assert.deepEqual(
    projectReleaseActivation(right),
    projectReleaseActivation(left),
  );
});
...
test("qualified resume never replays confirmed work and requires readback for uncertain work", () => {
  ...
  const state = foldReleaseActivation(request);
...
test("all rooted confirmations close the production plan under v4 authority", () => {
  ...
  const projection = projectReleaseActivation(request);
...
test("Rust and TypeScript produce byte-equivalent plans, folds, and roots", () => {
  ...
  const typescript = projectReleaseActivation(request);
...
test("ASCII step ordering is byte-stable across the Rust and TypeScript boundary", () => {
  ...
  const typescript = planReleaseActivation(request);
  assert.deepEqual(
