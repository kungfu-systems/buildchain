...
import { reportBuildchainIssue } from "../../packages/core/issue-reporting.js";
...
export async function resolvePlan() {
  ...
  const policy = await script("consumer-policy.mjs", {
  ...
  await script("validate-package-manager-contract.mjs", { BUILDCHAIN_PACKAGE_MANAGER_CWD: sourceRoot });
  let lockFailure;
  const lock = await script("buildchain-contract-lock.mjs", {
  ...
  const source = await script("resolve-publish-source.mjs", { ...commonEnv(plan), BUILDCHAIN_PUBLISH_SOURCE_SHA: plan.source.sha }, ["--mode", "lock"]);
  await script("verify-publish-channel-ref.mjs", { BUILDCHAIN_PUBLISH_SOURCE_REF: source["publish-source-ref"], BUILDCHAIN_PUBLISH_SOURCE_SHA: plan.source.sha, BUILDCHAIN_SOURCE_REPOSITORY: plan.run.repository });
  const manifest = await script("resolve-publish-source.mjs", { ...commonEnv(plan), BUILDCHAIN_SOURCE_CWD: path.join(sourceRoot, plan.project.cwd),
  ...
  if (plan.environment.runners.offline_fallback) {
    const routing = await script("route-offline-runners.mjs", { BUILDCHAIN_RUNNER_PRESET: "custom", BUILDCHAIN_PLATFORMS_JSON: runners.platformsJson,
  ...
  const controller = await script("controller-evidence.mjs", { BUILDCHAIN_CONTROLLER_ID: "build-lifecycle", BUILDCHAIN_CONTROLLER_SOURCE_REPOSITORY: plan.run.repository,
