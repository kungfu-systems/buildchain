...
export async function runStage() {
  ...
  try {
    ...
    if (stage === "install" && plan.anchored_material) {
      await script("anchored-version-material.mjs", {
    ...
    if (stage === "build" && usesCache) await script("compiler-cache-evidence.mjs", {}, ["prepare"], { cwd: sourceRoot });
    runLifecycle(lifecycleOptions(plan, platform, stage));
    if (stage === "build") {
      if (usesCache) await script("compiler-cache-evidence.mjs", {}, ["verify"], { cwd: sourceRoot });
    ...
    if (stage === "verify" && plan.build.macos_signing.app_path && platform.id === plan.build.macos_signing.platform) {
      await script("seal-macos-credential-input.mjs", { BUILDCHAIN_CREDENTIAL_ISLAND_APP_PATH: path.posix.join(plan.project.cwd, plan.build.macos_signing.app_path),
    ...
    if (stage === "verify" && platform.id === "linux-x64" && plan.build.transport_smoke.scenario_path) {
      await script("auditable-demo-transport-smoke.mjs", {}, ["--artifact-root", plan.build.transport_smoke.artifact_root, "--scenario", plan.build.transport_smoke.scenario_path], { cwd: path.join(sourceRoot, plan.project.cwd) });
    }
