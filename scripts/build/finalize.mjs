...
async function controllerReceipt(plan, jobs, success, executions = []) {
  ...
  const result = await script("controller-evidence.mjs", { BUILDCHAIN_CONTROLLER_PLAN_PATH: ".buildchain/controller/plan.json",
...
async function aggregate(plan, jobs, executions) {
  ...
  await script("aggregate-build-summary.mjs", env);
  await script("aggregate-diagnostics-summary.mjs", { BUILDCHAIN_DIAGNOSTICS_INPUT: ".buildchain/downloaded-diagnostics",
...
export async function finalizeBuild() {
  ...
  if (plan.build.artifacts.release_candidate && plan.source.candidate_channel !== "none") {
    ...
    await script("generate-release-candidate-passport.mjs", { ...aggregated.env,
