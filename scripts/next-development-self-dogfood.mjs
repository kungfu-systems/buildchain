...
export function validateSelfDogfoodCheckpoint(checkpoint) {
  if (checkpoint?.contract !== NEXT_DEVELOPMENT_SELF_DOGFOOD_CHECKPOINT) {
    throw new Error(
      "next-development self-dogfood checkpoint contract mismatch",
    );
  }
  ...
  const semver = validateNextDevelopmentController(
    checkpoint.semver.controller,
  );
  const anchored = validateNextDevelopmentController(
    checkpoint.anchored.controller,
  );
  if (
    ...
    throw new Error(
      "semver checkpoint must prove one transient durable-state failure",
    );
  }
  ...
  if (
    ...
    nextDevelopmentRoot(checkpointBody(checkpoint))
  ) {
...
export async function createSelfDogfoodCheckpoint({
  ...
  if (
    ...
    throw new Error(
      "self-dogfood checkpoint requires the public kungfu-systems/buildchain v4-alpha route",
    );
  }
  ...
  await runNextDevelopmentController(
    ...
    (error) => {
      if (
        !/injected transient durable-state write failure/u.test(error.message)
      ) {
        throw error;
      }
    },
...
function assertVerifiedModel(model, label) {
  ...
  if (
    ...
    throw new Error(
      `${label} does not bind its verified controller and readback`,
    );
  }
  return controller;
}
...
export function validateSelfDogfoodEvidence(evidence) {
  ...
  const semver = assertVerifiedModel(
    evidence.models?.semverAuto,
    "semver/auto",
  );
  const anchored = assertVerifiedModel(
    evidence.models?.anchoredManual,
    "anchored/manual",
  );
  if (evidence.status !== "passed")
    throw new Error("self-dogfood evidence did not pass");
  validateRecoveryEvidence(evidence.recovery);
  if (
    semver.controllerRoot !== evidence.transactionRoot ||
    nextDevelopmentRoot(semver.readback) !==
      evidence.protectedDevReadbackRoot ||
    evidence.models.anchoredManual.waitingStatus !== "waiting-anchor" ||
    evidence.models.anchoredManual.waitingControllerRoot ===
      anchored.controllerRoot
  ) {
...
export async function resumeSelfDogfoodCheckpoint(
  checkpointInput,
  { runnerId } = {},
) {
  ...
  const anchoredExecutor = new VersionStateAdapterHarness({
    route: checkpoint.route,
  });
  ...
  const evidence = {
    ...
    recovery: {
      ...
      protectedPrDelay: {
        ...
        unchangedControllerRoot:
          pending.controllerRoot === delayed.controllerRoot,
      },
    },
    adapter: {
...
function normalizeHostedReadback(value, route) {
  ...
  if (
    JSON.stringify(paths.map((entry) => entry.path)) !==
    JSON.stringify(route.hostedReadbackPaths)
  ) {
    throw new Error(
      "hosted protected Dev readback must exactly cover declared paths",
    );
  }
  const body = {
  ...
  if (
    body.repository !== route.repository ||
    body.branch !== route.protectedDevBranch
  ) {
...
export function createHostedSelfDogfoodEvidence({
  nextDevelopment,
  observation,
} = {}) {
  ...
  const failures = [];
  if (observed.alpha?.ref !== "v4-alpha")
    failures.push("alpha runtime ref mismatch");
  if (observed.alpha?.class !== "alpha")
    failures.push("alpha runtime class mismatch");
  if (observed.alpha?.sha !== observed.alpha?.expectedSha)
    failures.push("alpha runtime SHA mismatch");
  if (observed.alpha?.sha !== proof.route.runtimeSha)
    failures.push("next-development runtime SHA mismatch");
  if (observed.stable?.ref !== "v4")
    failures.push("stable runtime ref mismatch");
  if (observed.stable?.class !== "stable")
    failures.push("stable runtime class mismatch");
  if (observed.stable?.sha !== observed.stable?.expectedSha)
    failures.push("stable runtime SHA mismatch");
...
export function validateHostedSelfDogfoodEvidence(evidence) {
  ...
  if (
    readback.evidenceRoot !==
      evidence.hostedProtectedDevReadback?.evidenceRoot ||
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "verify") {
    return validateSelfDogfoodEvidence(
      readJson(required(options.input, "input")),
    );
  }
  if (command === "verify-hosted") {
    return validateHostedSelfDogfoodEvidence(
      readJson(required(options.input, "input")),
    );
  }
  throw new Error(
