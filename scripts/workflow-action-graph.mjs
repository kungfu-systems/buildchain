...
export function localActionDirectory(uses) {
  ...
  const match = /^\.\/(?:\.buildchain\/[a-z0-9][a-z0-9-]*\/)?(actions\/[a-z0-9-]+\/[a-z0-9-]+)$/u.exec(uses);
  return match?.[1] || null;
}
...
function excludedPhase(step, inputs) {
  ...
  return phase && typeof inputs.phase === "string" && !inputs.phase.includes("${{") && inputs.phase !== phase;
}
export function inspectWorkflowJob(relative, jobId, root = repositoryRoot) {
  ...
  function sourceModule(file) {
    ...
    assert.ok(!path.posix.isAbsolute(file) && !file.startsWith("../"), `Module escapes repository: ${file}`);
    ...
    function visit(node) {
      ...
      else if (ts.isCallExpression(node) && (node.expression.kind === ts.SyntaxKind.ImportKeyword || node.expression.getText(source) === "require")) value = node.arguments[0];
      if (value && ts.isStringLiteral(value) && value.text.startsWith(".")) {
        const dependency = path.posix.normalize(path.posix.join(path.posix.dirname(file), value.text));
  ...
  function walk(step, ancestry = [], inputs = {}) {
    ...
    if (!directory) return;
    assert.ok(!ancestry.includes(directory), `Composite action cycle: ${[...ancestry, directory].join(" -> ")}`);
    const action = readWorkflow(`${directory}/action.yml`, root);
    actions.set(directory, action);
    const childInputs = Object.fromEntries(Object.entries(step.with || {}).map(([key, value]) => [key, value === "${{ inputs.phase }}" ? inputs.phase : value]));
    if (action.runs.using === "composite") {
      for (const child of action.runs.steps) {
        if (excludedPhase(child, childInputs)) continue;
        const executableText = [child.run, child.with?.script].filter(Boolean).join("\n");
        for (const match of executableText.matchAll(/(?:\$GITHUB_ACTION_PATH\/|["'](?:\.\.\/)+)([^"'\s]+\.[cm]?js)/gu)) {
          const prefix = match[0].startsWith("$GITHUB_ACTION_PATH/") ? "" : match[0].slice(1, -match[1].length);
          sourceModule(path.posix.join(directory, prefix, match[1]));
        }
        for (const match of executableText.matchAll(/(?:^|[\s"'/])((?:packages\/core|scripts)\/[^\s"']+\.[cm]?js)/gu)) {
          const file = match[1];
      ...
      if (action.runs.post) sourceModule(`${directory}/${path.posix.basename(action.runs.post)}`);
