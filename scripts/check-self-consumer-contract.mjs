...
export function checkSelfConsumerContract(root = ROOT) {
  ...
  // The published bootstrap compiler admits feature/* only. Other task routes
  // are enabled after publishing the repaired compiler, before final closeout.
  for (const prefix of ["feature"])
    if (
      !plan.channels.some(
        (route) =>
          route.from === `${prefix}/*` &&
