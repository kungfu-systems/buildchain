...
- `releaseEvidence`: product-owned JSON evidence indexes copied into the
  passport as public release assets. Buildchain does not interpret their
  product or legal meaning; it binds each document's canonical JSON digest and
  exact source SHA, tag, and channel to the passport and verifies it again.
...
buildchain collect github-release \
  ...
  --release-evidence-json .buildchain/release-evidence/ungfu-public-use.json \
  --output-dir .buildchain/release-passport
```

Each repeated `--release-evidence-json` input must be a checked-in or generated
JSON file with `schemaVersion: 1`, a stable `id`, a product-owned `contract`,
and `release.sourceSha`, `release.tag`, and `release.channel` equal to the
passport. Collection copies it to
`release-evidence-<id>.json`, records its canonical SHA-256 in
`buildchain.release.json`, and fails closed on any coordinate mismatch.
`verify release-passport` retrieves and rehashes every declared document. The
same files are included recursively in `buildchain-release-bundle.tar.gz`, so
the public release retains both an independently downloadable index and an
