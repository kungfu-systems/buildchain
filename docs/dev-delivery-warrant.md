...
dequeue closes atomically and dispatches the rooted queued successor. Queued
...
duplicate terminal event or a pull request that never entered Warrant authority
returns a rooted explicit no-op instead of failing the workflow or inventing
queue state.
