...
function assertPackageManager(manager) {
...
function commandForRunScript(manager, script) {
...
function commandForVersion(manager, keyword, options = {}) {
  assertPackageManager(manager);
  const args = [];
  if (manager === "yarn") {
...
function commandForKungfuUpgrade(manager, scope = "@kungfu-trader") {
  assertPackageManager(manager);
  if (manager === "pnpm") {
    return {
      primary: shellJoin({
...
function getWorkspaceInfo(cwd = process.cwd()) {
...
function getYarnLockInfo(content) {
  if (!content) {
    return undefined;
  }
  const acc = new Map();
...
function getPnpmLockInfo(content) {
  const acc = new Map();
...
function getNpmLockInfo(content) {
...
function getCurrentLockInfo(cwd = process.cwd()) {
