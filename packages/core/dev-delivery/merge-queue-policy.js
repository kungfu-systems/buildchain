...
function normalizeRulesetBypassActors(actors = []) {
  ...
  for (const actor of actors) {
    ...
    if (!RULESET_BYPASS_TYPES.has(actorType)) {
      throw new Error(
        `ruleset bypass actor type must be Integration, Team, or User, got '${actorType}'`,
      );
    }
...
export async function resolveRulesetBypassActors({
  ...
  for (const slug of apps) {
    const app = await api.request(
      "GET",
      `apps/${encodeURIComponent(requiredString(slug, "bypass app"))}`,
    );
  ...
  for (const login of users) {
    const user = await api.request(
      "GET",
      `users/${encodeURIComponent(requiredString(login, "bypass user"))}`,
    );
...
export function selectMergeQueueMethod(repositorySettings = {}) {
  ...
  if (!selected) {
    throw new Error(
      "repository must allow at least one merge method before merge queue enablement",
    );
  }
  return selected;
}
...
function assertMergeQueueMethod(method, repositorySettings = {}) {
  ...
  if (!(normalized in allowed) || !allowed[normalized]) {
    throw new Error(
      `inherited merge queue method '${normalized}' is not enabled by the repository`,
    );
  }
  return normalized;
}
...
export function validateMergeGroupWorkflows(workflows = []) {
  if (workflows.length === 0) {
    throw new Error(
      "at least one required workflow must be declared with --workflow",
    );
  }
  const results = workflows.map(({ path: workflowPath, source }) => {
    ...
    return {
      ...
      hasPullRequest,
      hasMergeGroup,
      usesPullRequestPayload,
    };
  });
  const invalid = results.filter(
    (workflow) => !workflow.hasPullRequest || !workflow.hasMergeGroup,
  );
  if (invalid.length > 0) {
    throw new Error(
  ...
  const payloadBound = results.filter(
    (workflow) => workflow.usesPullRequestPayload,
  );
...
export function createDevMergeQueuePlan({
  ...
  if (!/^dev\/v\d+\/v\d+\.\d+$/.test(normalizedBranch)) {
    throw new Error(
      `branch must be a Buildchain dev channel, got '${normalizedBranch}'`,
    );
  }
  const workflowChecks =
    workflows.length === 0 && allowInheritedWorkflowEvidence
      ? []
  ...
  if (requiredChecks.length === 0) {
    throw new Error(
      `protected branch ${normalizedBranch} must declare required status checks before merge queue enablement`,
    );
  }
    ...
    rules: [
      {
        ...
        parameters: {
          check_response_timeout_minutes: positiveInteger(
            queueParameters.check_response_timeout_minutes ||
          ...
          max_entries_to_merge: positiveInteger(
            queueParameters.max_entries_to_merge,
            "max entries to merge",
            1,
          ),
          merge_method: mergeMethod,
          min_entries_to_merge: positiveInteger(
            queueParameters.min_entries_to_merge,
            "min entries to merge",
            1,
          ),
          min_entries_to_merge_wait_minutes: Number.isInteger(
...
async function loadDetailedRulesets(api, repository) {
  ...
  for (const ruleset of Array.isArray(rulesets) ? rulesets : []) {
    ...
    details.push(
      await api.request("GET", `repos/${repository}/rulesets/${ruleset.id}`),
    );
  }
  return details;
}
...
function mergeQueueRuleParameters(ruleset) {
  return (
    ruleset?.rules?.find((rule) => rule?.type === "merge_queue")?.parameters ||
    {}
  );
}
...
export async function resolveConfiguredDevMergeQueuePolicy({
  ...
  if (!sourceBranch || sourceBranch === branch) {
    const targetRuleset = findExactMergeQueueRuleset(
      await loadDetailedRulesets(api, repository),
      branch,
    );
    if (!targetRuleset) {
      return {
  ...
  const sourceRuleset = findExactMergeQueueRuleset(
    await loadDetailedRulesets(api, repository),
    sourceBranch,
  );
  return {
...
export async function reconcileConfiguredDevMergeQueue({
  ...
  const workflows = loadWorkflowSources(
    cwd,
    resolution.policy.requiredWorkflows,
  );
  const result = await reconcileDevMergeQueue({
    ...
    bypassActors:
      resolution.mode === "inherit"
        ? resolution.inheritedBypassActors
        : undefined,
    ...
    queueParameters:
      resolution.mode === "inherit" ? resolution.queueParameters : {},
    allowInheritedWorkflowEvidence:
      resolution.mode === "inherit" &&
      resolution.policy.requiredWorkflows.length === 0,
  });
