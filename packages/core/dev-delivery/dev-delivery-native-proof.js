...
function normalizeQualifiedWarrant(warrant) {
  for (const field of [
    "nativeProofRoot",
    ...
    warrant[field] = exactRoot(warrant[field], "Warrant " + field);
...
export function validateActiveDevDeliveryWarrant(queue) {
  ...
  const phase = warrant.phase;
  if (!["ready", "provisional", "qualified"].includes(phase))
  ...
  validateWarrantPhaseEvidence({ active, phase, warrant });
...
export function verifyNativeQualificationProof(proofInput, expected = {}) {
  try {
    ...
    if (
      ...
      executionReceipt.executionBindingRoot !== proof.nativeExecutionBindingRoot
    ) {
...
function validateWarrantPhaseEvidence({ active, phase, warrant }) {
  if (warrant.phase === "qualified") {
    normalizeQualifiedWarrant(warrant);
  } else if (
  ...
  if (
    phase === "ready" &&
    (active[0].deliveryClass !== "non-native-fast" ||
    ...
    throw new Error("ready Warrant must use the exact non-native contract");
  }
}
