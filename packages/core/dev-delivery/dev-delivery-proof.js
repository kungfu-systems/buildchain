...
export const REUSABLE_SOURCE_QUALIFICATION_PROOF_SCHEMA = "kungfu.buildchain.source-qualification-proof/v2";
...
export function createReusableSourceQualificationProof(input = {}) {
  const body = {
    schema: REUSABLE_SOURCE_QUALIFICATION_PROOF_SCHEMA,
    rootSemantics: SOURCE_QUALIFICATION_ROOT_SEMANTICS,
...
export function verifySourceQualificationProof(proofInput, expected = {}) {
  try {
    ...
    if (![SOURCE_QUALIFICATION_PROOF_SCHEMA, REUSABLE_SOURCE_QUALIFICATION_PROOF_SCHEMA].includes(proof.schema)) return { ok: false, reason: "unsupported-schema" };
    ...
    if (proof.schema === REUSABLE_SOURCE_QUALIFICATION_PROOF_SCHEMA) {
      if (proof.authority !== "exact-source-qualification") return { ok: false, reason: "non-exact-authority" };
      createReusableSourceQualificationProof(proof);
    } else {
...
export function classifyDevDeliveryDelta({ proof, current = {} } = {}) {
  ...
  const exactPredicates = ["sourceIdentityRoot", "sourcePatchRoot", "planRoot", "closureRoot", "dependencyRoot", "toolchainRoot", ...(proof.schema === REUSABLE_SOURCE_QUALIFICATION_PROOF_SCHEMA ? ["qualifiedBase", "sourceHead", "sourceTree", "policyRoot", "requiredContextRoot"] : [])];
