...
} from "../../release/commands/release-candidate-resolver.mjs";
...
function executeBootstrapConformance(request, admission) {
  ...
  for (const relative of architecture.bootstrapGovernedWorkflows) {
    const source = fs.readFileSync(
      path.join(path.resolve(import.meta.dirname, "../../../.."), relative),
      "utf8",
    );
    if (
  ...
  return {
    ...
    engineRoot: rawFileRoot("packages/core/workflow/commands/universal-workflow-engine.mjs"),
  };
}
...
async function materializeProductPublicationIntent({
  ...
  execFileSync(
    ...
    [
      path.resolve(
        ".buildchain/candidate/packages/core/release/commands/product-publication-intent.mjs",
      ),
    ],
    {
      env: {
