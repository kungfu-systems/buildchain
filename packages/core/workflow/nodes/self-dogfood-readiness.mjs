...
export default async function readSelfDogfoodReadiness(
  { github, core },
  env = process.env,
) {
  const number = Number(env.REVIEW_PR),
  ...
  core.setOutput(
    "ready",
    String(
      selfDogfoodReady({ pullRequest, reviews, checks, candidateSha, number }),
    ),
  );
}
