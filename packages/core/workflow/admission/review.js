...
export function reviewedCandidateEvidence({
  ...
  for (const review of [...reviews].sort(
    (a, b) =>
      Date.parse(exactTime(a.submitted_at)) -
      Date.parse(exactTime(b.submitted_at)),
  )) {
  ...
  for (const check of [...checks].sort((a, b) => (a.id || 0) - (b.id || 0))) {
    if (check.head_sha)
      requireValue(
        check.head_sha === candidate.expectedSha,
        "Check belongs to a different candidate",
      );
...
export function readCandidateReview(
  { request, workspace, runtimeRoot, candidateRoot, token },
  execute = command,
) {
  if (!token)
    throw new Error("Candidate review requires an explicit read credential");
  const { repository, expectedSha, reviewPullRequest } = request.candidate;
  const observed = execute("git", ["-C", candidateRoot, "rev-parse", "HEAD"], {
  ...
  fs.mkdirSync(path.join(workspace, ".buildchain"), { recursive: true });
  for (const [name, value] of Object.entries({
    ...
    fs.writeFileSync(
      path.join(workspace, `.buildchain/${name}.json`),
  ...
  return reviewedCandidateEvidence({
    ...
    checks,
    commit,
    line: bootstrapVersionLine(runtimeRoot),
