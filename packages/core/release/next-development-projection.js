...
adapter = "packages/core/release/commands/next-development-transition.mjs"
...
\`.github/workflows/buildchain.yml\` calls the published
\`public-ops-pipeline.yml@v4\` entry. Products declare their verification commands
in schema-2 TOML, including the full repository check on each required platform.
Next-development preparation, independent review, protected merge and settlement
remain runtime responsibilities.

\`.github/workflows/buildchain-recover.yml\` calls
\`public-ops-recover.yml@v4\` with one exact attempt and an optional temporary
repaired runtime. It preserves completed publication evidence while continuing
unfinished follow-ups. Both callers use the same generated bytes as external
consumers. Tool-maintained locks and retained receipts bind exact runtime SHAs.
Earlier Stage Capsule and channel-build proofs retain their historical source
identities; they do not define additional current consumer entrypoints.
