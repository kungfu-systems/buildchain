...
import { ensureGitHubRelease } from "./commands/ensure-github-release.mjs";
import {
...
export async function recoverCompleteGitHubReleaseEvidence({
  octokit,
  owner,
  repo,
  token,
  ...
  const release = await ensureGitHubRelease({
    ...
    tag,
    title: tag,
    notes: `Buildchain release passport assets for ${tag}.`,
    target,
    channel,
  });
