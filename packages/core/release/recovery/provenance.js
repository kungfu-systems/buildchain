...
export function resolveAnchorRecoveryRequest({
  passport,
  buildSummary,
  transactionId = "",
} = {}) {
  ...
  if (
    passport?.target?.channel !== "anchor" ||
    passport?.target?.ref !== "publish-gate/anchor"
  ) {
  ...
  try {
    releaseManifest = JSON.parse(
      buildSummary?.publishSource?.releaseManifest || "",
    );
  } catch {
  ...
  if (
    ...
    request?.schema !== 1 ||
    request?.contract !==
      "kungfu-buildchain-explicit-publish-anchor-request/v1" ||
...
function normalizeRun(run, repository) {
  return {
    ...
    pullRequestNumbers: (run.pull_requests || []).map((entry) =>
      Number(entry.number),
    ),
  };
}
...
export async function recoverCandidateProvenance({
  ...
  const request = resolveAnchorRecoveryRequest({
    passport,
    buildSummary,
    transactionId,
  });
  ...
  if (
    ...
    throw new Error(
      "superseded candidate run has more than 100 artifacts; complete pagination is required before recovery",
    );
  ...
  const passportDownload = await downloadArtifact({
    artifact: selected.passport,
    repoInfo,
    apiUrl,
    token,
    archiveDir,
    bundleRoot: provenanceRoot,
    fetchImpl,
  });
  const summaryDownload = await downloadArtifact({
  ...
  const recoveredPassport = readOnlyJson(
    passportDownload.files.filter(
      (file) => path.basename(file.path) === "release-candidate-passport.json",
    ),
    "superseded release-candidate-passport.json",
  );
  const recoveredBuildSummary = readOnlyJson(
    summaryDownload.files.filter(
      (file) => path.basename(file.path) === "build-summary.json",
    ),
    "superseded build-summary.json",
  );
  if (
    String(run.id) !== runId ||
    recoveredPassport.workflow?.runId !== runId ||
    recoveredPassport.workflow?.name !== expectedWorkflowName ||
    String(workflow.path || run.path || "")
      .split("@")[0]
      .replace(/^\.github\/workflows\//u, "") !==
      expectedWorkflowFile.replace(/^\.github\/workflows\//u, "") ||
    ...
    `sha256:${recoveredPassport.candidateHash}` !==
      request.supersededCandidate.root
  )
    throw new Error(
      "superseded candidate provenance does not match the anchored request",
    );
  const anchorProvenance = {
    ...
    buildSummary: recoveredBuildSummary,
  };
