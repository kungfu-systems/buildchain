...
class RefMutationOperations {
  ...
  async updateBranch(branch, branchSha, action = "updated", protectedUpdate) { const { COMMIT_IDENTITY, branchProtectionBypassApps, createGeneratedVersionStateChecks, cwd, dryRun, ensureManagedChannelBranchProtection, execFileSync, fs, getGitCommitWithRetry, getReconciliationOperations, getVersionStateOperations, nonFastForwardUpdateRejected, notFound, octokit, owner, path, protectedBranchDirectUpdateError, protectedBranchUpdateRejected, pullRequestOctokit, reconciliationWorkspace, refUpdateOctokit, repo, requiredStatusCheck, retryGitHubOperation, signedGeneratedCommitMessage, statusCheckOctokit, updates, versionStateBranchName } = this;
    if (dryRun) {
      updates.push({ ref: branch, action: "dry-run", sha: branchSha });
      return { updated: true };
    }
    const ensureChannelProtection = async () => {
      const policyEvidence = await ensureManagedChannelBranchProtection({ octokit, owner, repo, branch, requiredStatusCheck, branchProtectionBypassApps, });
