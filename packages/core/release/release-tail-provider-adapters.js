...
export function createGitHubReleaseAssetsAdapter({
  ...
  return Object.freeze({
    ...
    async apply(effect) {
      ...
      // Validate the complete immutable set before uploading any missing asset.
      for (const artifact of expected) {
        const existing = byName.get(artifact.name);
        if (existing && String(existing.digest || "").toLowerCase() !== artifact.root) {
          providerError(`immutable GitHub Release asset collision: ${artifact.name}`, {
      ...
      for (const artifact of expected) {
        if (byName.has(artifact.name)) continue;
        try {
