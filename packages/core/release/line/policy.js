...
export function isMajorGateRef(ref) {
  ...
  return normalizedRef === MAJOR_GATE_REF;
}

export function parseVersionStateRef(ref) {
  const normalizedRef = normalizeRef(ref);
  if (
    /^buildchain\/version-state\/publish-gate-major\/[0-9a-f]{12,40}$/.test(
      normalizedRef,
    )
  ) {
    return {
...
export function parseReleaseLineRecoveryRef(ref) {
  ...
  const match =
    ...
    normalizedRef.match(
      /^chore\/v4-product-pr\/(alpha|release)-v(\d+)-v(\d+\.\d+)\/[0-9a-f]{12}-[0-9a-f]{12}-[0-9a-f]{12}$/,
    );
  if (!match) return undefined;
  return {
    channel: match[1],
...
function getRecoveryBumpKeyword({
  target,
  baseRef,
  baseChannel,
  version,
  ...
  if (!target) return "";
  if (
    baseChannel !== target.channel ||
    target.normalizedRef !== normalizeRef(baseRef)
  ) {
    throw new Error(
      `Versions not match for head/base refs: ${headRef} -> ${baseRef}`,
    );
  }
  if (target.major !== version.major || target.loose !== looseVersionNumber) {
    throw new Error(
      `The version of head ref ${headRef} does not match current ${version.version}`,
    );
  }
...
export function parseAuthorityRef(ref) {
  const normalizedRef = normalizeRef(ref);
  const match = normalizedRef.match(
    /^authority\/v(\d+)\/v(\d+\.\d+)\/[A-Za-z0-9._/-]+$/,
  );
  if (!match) return undefined;
  return {
...
export function readCurrentVersion(cwd = process.cwd()) {
  const packagePath = path.join(
    cwd,
    fs.existsSync(path.join(cwd, "lerna.json")) ? "lerna.json" : "package.json",
  );
...
function authorityPromotionKeyword({
  authorityTarget,
  headRef,
  baseRef,
  version,
...
function validateBumpSourceVersion({
  headRef,
  version,
  looseVersionNumber,
  lastLooseVersionNumber,
  loose,
}) {
...
export function getBumpKeyword({
  cwd = process.cwd(),
  headRef,
  baseRef,
  loose = false,
  ...
  const lts =
    baseChannel === "release" &&
    normalizeRef(baseRef).split("/").pop() === "lts";
  const preminor = headChannel === "release" && lts;
  const majorGate =
    headChannel === "release" && baseChannel === MAJOR_GATE_CHANNEL;

  const authorityKeyword = authorityPromotionKeyword({
    authorityTarget,
    headRef,
    baseRef,
    version,
  ...
  if (authorityKeyword) return authorityKeyword;

  if (recoveryKeyword) return recoveryKeyword;

  if (publishGateTarget) {
    if (
      publishGateTarget.channel !== baseChannel ||
      publishGateTarget.normalizedRef !== normalizeRef(baseRef)
    ) {
      throw new Error(
        `Versions not match for head/base refs: ${headRef} -> ${baseRef}`,
      );
    ...
    const mismatchMsg = `The version of head ref ${headRef} does not match current ${version.version}`;
    if (
      publishGateTarget.major !== version.major ||
      publishGateTarget.loose !== looseVersionNumber
    ) {
      throw new Error(mismatchMsg);
    }
    return baseChannel === "release" ? "patch" : "prerelease";
  ...
  if (
    getLineSuffix(headRef, headChannel) !==
      getLineSuffix(baseRef, baseChannel) &&
    !preminor &&
    !majorGate
  ) {
    throw new Error(
  ...
  if (versionStateTarget) {
    ...
    return baseChannel === "release" || baseChannel === MAJOR_GATE_CHANNEL
      ? "patch"
      : "prerelease";
  }

  validateBumpSourceVersion({
    headRef,
    version,
    looseVersionNumber,
    lastLooseVersionNumber,
  ...
  if (!keyword) {
    throw new Error(
      `No rule to bump for head/base refs: ${headRef} -> ${baseRef}`,
    );
  }
  return keyword;
}
