...
import { recordPropagationStage } from "./work.js";

export function preparePropagationOutcome(context) {
  writePropagation(
    "pr-outcome.json",
    {
      state: "planned",
      number: null,
      url: "",
      branch: readPropagation("target.json", context).branch,
    },
    context,
  );
}
...
export function openPropagationPr(
  ...
  const input = request(context),
    { downstream, root } = propagationPaths(context);
  ...
  const gh = (args) =>
    execute("gh", args, {
      cwd: downstream,
      stdio: "pipe",
      env: { ...process.env, GH_TOKEN: context.token },
    }).trim();
  requireValue(
  ...
  if (changed || remoteSha) {
    ...
    writePropagation("push-result.json", pushResult, context);
    record(context, "push-branch", [pushResult.evidence]);
  }
  const bodyPath = path.join(root, "pr-body.md");
  fs.writeFileSync(
    bodyPath,
    `${input["pr-body"] || ""}\n\nBuildchain release propagation plan:\n\n\`\`\`json\n${JSON.stringify(readPropagation("plan.json", context), null, 2)}\n\`\`\`\n`,
  );
  ...
  writePropagation("pr-outcome.json", result, context);
  writePropagation(
