...
export async function settlePublication({
  ...
  const retained = release.assets.filter(
    ({ name }) => name === SETTLEMENT_ASSET,
  );
  if (retained.length > 1)
    throw new Error("ambiguous immutable publication settlement");
  if (retained.length === 1) {
    const original = JSON.parse(client.assetBytes(retained[0]));
    if (
      original.contract !== settlement.contract ||
      original.release?.sourceSha !== sourceSha ||
      original.release?.tag !== tag ||
