...
import { writeReleaseCandidatePassport } from "../candidate/passport.js";
...
export function generateReleaseCandidatePassportCli() {
  ...
  const { passport, stageCapsules } = writeReleaseCandidatePassport({ request, outputPath,
    coordinatesPath: path.resolve(env("BUILDCHAIN_ARTIFACT_COORDINATES_PATH", ".buildchain/artifacts/artifact-coordinates.json")), workspace: process.cwd() });
  writeGitHubOutputs({
