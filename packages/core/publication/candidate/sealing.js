import { resolveOciCandidate } from "./kind.js";
...
import { createPublicationSealedBundle } from "../publication-sealed-bundle.js";
...
export function createResolvedPublicationSealedBundle({
  ...
  const byAbsolutePath = new Map(
    files.map((file) => [path.resolve(file.absolutePath), file]),
  );
  const resolvedNpmArtifacts = npmArtifacts.map((artifact, index) => {
    const file = byAbsolutePath.get(
      path.resolve(
        requiredString(artifact.path, `npmArtifacts[${index}].path`),
      ),
    );
    if (!file)
      throw new Error(
        `npmArtifacts[${index}].path is outside the sealed payload root`,
      );
    return { file, metadata: artifact };
  });
  const main =
    resolvedNpmArtifacts.find((entry) => entry.metadata.role === "main") ||
    (resolvedNpmArtifacts.length === 1 ? resolvedNpmArtifacts[0] : undefined);
  if (
    !main ||
    resolvedNpmArtifacts.filter((entry) => entry.metadata.role === "main")
      .length > 1
  )
    throw new Error(
      "candidate npm payload set has no unique main package tarball",
    );
  const selectedReleaseAssets = (
    releaseAssetPaths.length > 0
      ? releaseAssetPaths
    ...
    if (!file)
      throw new Error(
        `releaseAssetPaths[${index}] is outside the sealed payload root`,
      );
    return file;
  });
  ...
  if (resolvedNpmArtifacts.length > 1) {
    candidatePayload.npmPackages = resolvedNpmArtifacts.map(
      ({ file, metadata }) => ({
        name: metadata.name,
        version: metadata.ref,
        role: metadata.role,
        path: file.path,
        size: file.size,
        sha256: file.sha256,
  ...
  return {
    root: resolvedRoot,
    manifest,
    npmArtifacts: resolvedNpmArtifacts,
    files,
  };
}

export function resolveCandidatePublicationBundle({
  kind,
  payloadDir,
  passport,
  runtimeSha,
  ...
  if (kind === "oci")
    return resolveOciCandidate({ payloadRoot: payloadDir, passport });
  if (kind !== "npm") return undefined;
  return createResolvedPublicationSealedBundle({
