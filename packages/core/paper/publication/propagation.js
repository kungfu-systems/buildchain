...
export async function capturePaperPropagation({
  ...
  const upstreamRelease = JSON.parse(
    fs.readFileSync(
      path.join(workspace, ".buildchain/upstream-release.json"),
      "utf8",
    ),
  );
  ...
  fs.writeFileSync(
    path.join(workspace, ".buildchain/release-propagation-config.json"),
    JSON.stringify(config, null, 2) + "\n",
  );
  fs.writeFileSync(
    path.join(workspace, ".buildchain/release-propagation-plan.json"),
  ...
  return {
    configured: true,
    "artifact-name": `paper-propagation-work-${packageVersion}-${sourceSha}`,
  };
}
