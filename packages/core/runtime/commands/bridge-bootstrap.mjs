...
function parseRunArgs(args) {
  ...
  if (!command) {
    throw new Error(
      "usage: node packages/core/runtime/commands/bridge-bootstrap.mjs <run|exchange> <command-id> -- [command-args...]",
    );
  }
...
function main(args = process.argv.slice(2)) {
  ...
  if (!["run", "exchange"].includes(mode)) {
    throw new Error(
      "usage: node packages/core/runtime/commands/bridge-bootstrap.mjs <build|run|exchange> ...",
    );
  }
