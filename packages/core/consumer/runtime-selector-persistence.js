...
import { parseYamlJsonInputs } from "../contracts/workflow-yaml-contract.js";
...
function jsonTemplateSource(text) {
  let quoted = false;
  let escaped = false;
  let result = "";
  for (let index = 0; index < text.length; index += 1) {
    if (text.startsWith("${{", index)) {
      ...
      if (end < 0) throw new Error("unterminated workflow expression");
      const expression = JSON.stringify(text.slice(index, end + 2));
      result += quoted ? expression.slice(1, -1) : expression;
      index = end + 1;
      continue;
    }
...
function yamlEnvelopeFailures(text, sourcePath) {
  return parseYamlJsonInputs(text).flatMap(({ key, line, value }) => {
    if (!["request-json", "invocation-json"].includes(key) || !value) return [];
    try {
      const envelope = JSON.parse(jsonTemplateSource(value));
      if (typeof envelope === "string" && transientSelector(envelope)) {
        return [
          {
            code: "persistent-runtime-json-indirection",
            path: sourcePath,
            line,
          },
        ];
      }
      return jsonSelectorFailures(envelope, sourcePath, [key]).map(
        (failure) => ({ ...failure, line }),
      );
    } catch (error) {
      return [
        {
          code: "runtime-selector-json-envelope-invalid",
          path: sourcePath,
          line,
...
function inspectSource({
  ...
  if (/\.ya?ml$/u.test(sourcePath)) {
    failures.push(...yamlEnvelopeFailures(text, sourcePath));
  }
  ...
  for (const [index, line] of lines.entries()) {
    if (RUNTIME_SELECTOR.test(line)) {
      ...
      if (/\b(?:train|authority)\/v4\/v4\.\d+\/[A-Za-z0-9._/-]+/u.test(line)) {
        failures.push({
          code: "persistent-runtime-transient-ref",
          path: sourcePath,
          line: index + 1,
        });
      }
      if (/\$\{\{[^}]*\b(?:vars|secrets|env)\./u.test(line)) {
        failures.push({
