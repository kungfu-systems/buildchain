...
import { parseYamlUses } from "../contracts/workflow-yaml-contract.js";
import {
...
function classifyBuildchainUses(
  records,
  failures,
  { repository, channel = "" } = {},
) {
  ...
  for (const record of records) {
    ...
    uses.push({
      ...
      selectorClass: selectedChannel ? "floating" : "rejected",
    });
    if (!selectedChannel)
      failures.push(selectorFailure(record, parsed.selector));
  }
  return uses;
}
...
export function scanFloatingConsumerPolicy({
  ...
  const buildchainUses = classifyBuildchainUses(scanned.records, failures, {
    repository,
    channel: expectedInvocationChannel,
  });
