...
export function scanCommand(options = {}) {
  ...
  const result = scanFloatingConsumerPolicy({
    ...
    definitionRepository:
      options.definitionRepository || env("BUILDCHAIN_DEFINITION_REPOSITORY"),
    definitionSha: options.definitionSha || env("BUILDCHAIN_DEFINITION_SHA"),
...
export function certifyCommand(options = {}) {
  ...
  const authorityScan = scanFloatingConsumerPolicy({
    ...
    invocationRoot:
      ...
      env("BUILDCHAIN_INVOCATION_SOURCE_ROOT", callerRoot),
    definitionRepository:
      options.definitionRepository || env("BUILDCHAIN_DEFINITION_REPOSITORY"),
    definitionSha: options.definitionSha || env("BUILDCHAIN_DEFINITION_SHA"),
    repository,
