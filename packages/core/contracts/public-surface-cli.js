...
export function enumerateCliCommandsFromBin({
  ...
  const dispatchSource = [
    binSource,
    ...fs
      .readdirSync(path.join(root, "packages/core"), { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .flatMap((entry) =>
        listFiles(root, `packages/core/${entry.name}/cli`, (name) =>
          name.endsWith(".mjs"),
        ),
      )
  ...
  const registered = [...registrySource.matchAll(/\{\s*id:\s*"([^"]+)"/g)].map(
    (match) => commandId(match[1]),
  );
  return uniqueSorted([
    ...
    usage: usage.find((entry) => entry.id === id)?.usage || `buildchain ${id}`,
  }));
}
