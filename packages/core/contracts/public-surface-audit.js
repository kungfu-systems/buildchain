import { actionInventory } from "./action-inventory.js";
...
export function enumerateActionInputs({ root = process.cwd() } = {}) {
  return actionInventory(root).map(({ directory: dir }) => {
...
export function enumerateDocCommandRefs({ root = process.cwd() } = {}) {
  ...
  const docs = [
    ...
    ...actionInventory(root).map(({ directory }) => `${directory}/README.md`).filter((relPath) => fs.existsSync(path.join(root, relPath))),
  ];
  const refs = [];
