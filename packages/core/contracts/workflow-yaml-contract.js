...
export function parseYamlJsonInputs(text) {
  const lines = String(text || "").split(/\r?\n/u);
  const inputs = [];
  let block = null;
  for (const [index, line] of lines.entries()) {
    const indent = indentation(line);
    if (block) {
      if (!line.trim() || indent >= block.contentIndent) continue;
      block = null;
    }
    const mapping = line.match(
      /^(\s*)(?:-\s+)?(?:"([A-Za-z0-9_.-]+)"|'([A-Za-z0-9_.-]+)'|([A-Za-z0-9_.-]+)):(?:\s*(.*))?$/u,
    );
    if (!mapping) continue;
    const key = mapping[2] || mapping[3] || mapping[4];
    const value = stripComment(mapping[5] || "");
    if (key.endsWith("-json")) {
      inputs.push({
        key,
        line: index + 1,
        value: unquote(blockValue(lines, { value, line: index, indent })),
      });
    }
    if (/^[>|][+-]?$/u.test(value)) {
