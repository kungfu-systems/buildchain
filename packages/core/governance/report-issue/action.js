...
function setResultOutputs(result) {
  ...
  core.setOutput(
    "issue-number",
    result.issueNumber ? String(result.issueNumber) : "",
  );
...
function recordWorkflowFrictionOutcome(result) {
  if (fallbackReport?.contract !== BUILDCHAIN_WORKFLOW_FRICTION_ISSUE_CONTRACT)
    return;
  const outcome =
...
async function main() {
  ...
  const bodyParts = [input("body"), readOptionalIssueBodyFile(bodyFile)].filter(
    Boolean,
  );
  const common = {
    token: input("token"),
