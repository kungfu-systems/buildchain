...
import { dependencyCycles } from "../architecture-graph.js";
...
function validateCapabilities(capabilities, inventory, issues) {
  ...
  for (const [index, capability] of (capabilities || []).entries()) {
    ...
    if (!source)
      issues.push(`${capability?.id} has unknown current source inventory id`);
    else {
  ...
  for (const id of inventoryById.keys()) {
    if (!(capabilities || []).some((entry) => entry.sourceInventoryId === id))
      issues.push(
        `current mechanism is absent from capability manifest: ${id}`,
      );
  }
...
function validateManifest({ manifest, inventory, bootstrap, ledger }) {
  ...
  if (manifest?.releaseLine?.branch !== "dev/v4/v4.1")
    issues.push("releaseLine.branch must be dev/v4/v4.1");
  ...
  if (
    manifest?.releaseLine?.rollbackRef !==
    "1bb6333b97ad94e94a81178e956a703dff2b5f84"
  )
    issues.push(
      "releaseLine.rollbackRef must retain 1bb6333b97ad94e94a81178e956a703dff2b5f84",
    );
  if (
  ...
  // Bootstrap is immutable historical evidence; current development authority is
  // the manifest's exact protected seed, never a retained legacy execution lane.
  if (manifest?.releaseLine?.bootstrapAuthority !== BOOTSTRAP_PATH)
    issues.push("manifest must retain the exact bootstrap evidence source");
...
function loadArchitecture(root = process.cwd()) {
  ...
  const inventory = readJson(root, "architecture/core-mechanisms.json");
  const schemaIssues = [];
