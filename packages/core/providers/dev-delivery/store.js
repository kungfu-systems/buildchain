...
import { normalizeDevDeliveryAuthorityState } from "../../dev-delivery/dev-delivery-authority-state.js";
...
export class GitHubDevDeliveryStore {
  ...
  async readOnce({ stateRef, protectedBase, now }) {
    let ref;
    try {
  ...
  async readCommit(commitShaInput) {
    return this.retryParseableRead(async () =>
      this.readCommitOnce(commitShaInput),
    );
  }

  async readCommitOnce(commitShaInput) {
