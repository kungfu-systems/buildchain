...
} from "./codebuild-policy.js";
function installCompiler(run, profile, elevated) {
...
export function prepareAwsCodeBuildToolchain({
  ...
  function prepare() {
    ...
    installCompiler(run, compiler, elevated);
    ...
    const cmakeVersion = run(cmakePath, ["--version"], {
      capture: true,
    }).trim();
    ...
    return { evidence, paths: [aliasDirectory, cmakeBinDirectory] };
  }
