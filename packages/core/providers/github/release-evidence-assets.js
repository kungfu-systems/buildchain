...
function selectedAssets(assets, names) {
...
export async function readReleaseEvidenceAssets(host, candidate, names) {
  ...
  const selected = selectedAssets(await inventory(host.request, prefix), names);
  ...
  for (const asset of selected) {
    ...
    values[asset.name] = bytes;
  }
  if (
    recordDigest(
      selectedAssets(await inventory(host.request, prefix), names),
    ) !== recordDigest(selected)
  )
    throw new Error(
      "Published evidence assets changed during provider readback",
    );
  return { status: "present", values, assets: selected };
}
