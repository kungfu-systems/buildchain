...
function assertLivePreflight(plan, profile) {
  ...
  return {
    budgetGuard: jsonResult(commandResult("bash", ["packages/core/providers/commands/aws-windows-jit-operator.sh", "launch-gate", "--region", plan.aws.region, ...(profile ? ["--aws-profile", profile] : [])]), "provider Budget launch gate"),
