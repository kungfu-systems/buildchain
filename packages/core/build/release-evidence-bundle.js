...
export const BUNDLE_CONTRACT = "kungfu-buildchain-release-evidence-bundle";
...
export function createReleaseEvidenceBundle({
  ...
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "buildchain-release-bundle-"),
  );
  const bundleRoot = path.join(tempDir, BUNDLE_BASE);
  const sourceSets = [
    {
      id: "release-assets",
      dir: resolvedAssetsDir,
      bundlePrefix: "release-assets",
    },
    {
      id: "release-passport",
      dir: resolvedPassportDir,
      bundlePrefix: "release-passport",
    },
  ];
  const included = [];
  ...
  fs.writeFileSync(
    path.join(bundleRoot, `${BUNDLE_BASE}.index.json`),
