...
import { runProcessTreeCommandSync, runShellCommandSync } from "../../runtime/spawn-command.js";
import {
...
function resolveBuildchainCliPath() {
  const entry = path.resolve(moduleDir, "../../../../bin/buildchain.mjs");
  if (!fs.existsSync(entry)) throw new Error("Buildchain distribution is missing its CLI entrypoint");
  return entry;
}
