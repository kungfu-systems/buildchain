...
export function importArtifactSigningResults({
  ...
  for (const entry of requestIndex.requests || []) {
    const request = JSON.parse(
      fs.readFileSync(
        resolveBelow(requests, entry.path, "request path"),
        "utf8",
      ),
    );
    const check = validateArtifactSigningRequest(request);
    if (!check.ok || request.digest !== entry.digest)
      throw new Error(`invalid request during result import: ${entry.id}`);
  ...
  for (const entry of resultIndex.results || []) {
    ...
    if (!request)
      throw new Error(`result has no local sealed request: ${entry.id}`);
    const resultPath = resolveBelow(results, entry.result, "result path");
    const result = JSON.parse(fs.readFileSync(resultPath, "utf8"));
    const payload = resolveBelow(
    ...
    if (rel.startsWith("..") || path.isAbsolute(rel))
      throw new Error("imported signing evidence escapes consumer source root");
    fs.mkdirSync(path.dirname(destination), { recursive: true });
    fs.cpSync(path.dirname(resultPath), destination, {
      recursive: true,
      force: false,
      errorOnExist: true,
    });
  ...
  return { ...verification, imported, credentialArtifacts, outputs };
}
