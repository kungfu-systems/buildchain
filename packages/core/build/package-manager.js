...
function expandWorkspacePattern(cwd, pattern) {
  ...
  const directories = parts.reduce((parents, part) => parents.flatMap(parent => {
    if (!fs.existsSync(parent) || !fs.statSync(parent).isDirectory()) return [];
    if (part !== "*") return [path.join(parent, part)];
    return fs.readdirSync(parent, { withFileTypes: true })
      .filter(entry => entry.isDirectory())
      .map(entry => path.join(parent, entry.name));
  }), [cwd]);
  return directories.filter(dir => fs.existsSync(path.join(dir, "package.json")));
}
