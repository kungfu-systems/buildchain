...
function listBundleFiles(root, prefix = "") {
  ...
  for (const entry of entries) {
    requireValue(
      ...
      `bundle member must not be a symbolic link: ${path.join(prefix, entry.name)}`,
    );
...
export function verifyBundleChecksums(root, label, options = {}) {
  ...
  const checksums = readRegular(
    path.join(resolved, "checksums.sha256"),
    `${label} checksums`,
  );
  ...
  for (const row of checksums.toString("utf8").split("\n").filter(Boolean)) {
    ...
    requireValue(
      ...
      `${label} checksum member is repeated`,
    );
    declared.add(match[2]);
    const metadataMember =
      match[2].endsWith(".json") || match[2].endsWith(".sha256");
    const maximum =
      options.allowLongFormRendererManifest && match[2] === "manifest.json"
    ...
    requireValue(
      bundleBytes <= options.maximumBundleBytes,
      `${label} exceeds its aggregate byte budget`,
    );
    members.push({
      expectedRoot: `sha256:${match[1]}`,
      maximum,
      name: match[2],
      target,
    });
  ...
  for (const member of members) {
    const verified =
      options.allowLongFormRendererManifest && member.name === "manifest.json"
        ? (() => {
            const value = readRendererManifest(
              member.target,
              RENDERER_MANIFEST_HELPERS,
            ).bytes;
            return { bytes: value.length, root: rootBytes(value) };
          })()
        : digestRegular(member.target, `${label} member`, member.maximum);
    requireValue(
      verified.root === member.expectedRoot,
      `${label} checksum mismatch: ${member.name}`,
    );
    if (member.name === "manifest.json" && options.rendererManifestRoot) {
      requireValue(
        verified.root === options.rendererManifestRoot,
        `${label} renderer manifest root mismatch`,
      );
    }
  }
  const actual = listBundleFiles(resolved).filter(
    (name) => name !== "checksums.sha256",
  );
  requireValue(
    JSON.stringify([...declared].sort()) === JSON.stringify(actual),
    `${label} checksum member set is not exact`,
  );
...
export function copyVerifiedRegular(source, destination, label, maximum) {
  ...
  const destinationDigest = digestRegular(
    destination,
    `${label} copy`,
    maximum,
  );
  requireValue(
    destinationDigest.bytes === sourceDigest.bytes &&
      destinationDigest.root === sourceDigest.root,
    `${label} copy differs from its verified source`,
  );
  return {
    path: path.basename(destination),
    bytes: destinationDigest.bytes,
    root: destinationDigest.root,
  };
}
