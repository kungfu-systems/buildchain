...
export function readRendererManifest(filePath, helpers) {
  ...
  try {
    ...
    if (error instanceof SyntaxError)
      throw new Error("renderer manifest must be valid JSON");
    throw error;
  }
  invariant(
    manifest.schema === "build-images.auditable-demo-render/v1",
    "unexpected renderer manifest schema",
  );
  if (bytes.length > MAX_BUNDLE_MEMBER_BYTES) {
    const renditions = manifest.inputs?.renditions;
    invariant(
      Array.isArray(renditions) && renditions.length === 2,
      "oversized renderer manifest requires exactly two bounded long-form native renditions",
    );
    const expected = [
      { id: "1080p", role: "primary", width: 1920, height: 1080 },
      { id: "720p", role: "responsive", width: 1280, height: 720 },
    ];
    renditions.forEach((entry, index) =>
      validateLongFormManifestRendition(entry, expected[index], index, helpers),
    );
  }
...
export function validateTerminalCapture(value, scene, helpers) {
  ...
  invariant(
    value.schema === "kungfu.terminal-capture/v1",
    "unsupported terminal capture schema",
  );
  ...
  const maximumDurationMs =
    scene.durationClass === "long-form" ? 180000 : 60000;
  const durationMs = integer(
    value.durationMs,
    500,
    maximumDurationMs,
    "terminalCapture.durationMs",
  );
  invariant(
  ...
  invariant(
    ...
    "terminalCapture.encoding must be base64",
  );
  invariant(
  ...
  for (const [index, event] of value.events.entries()) {
    ...
    const atMs = integer(
      ...
      0,
      durationMs - 1,
      `terminalCapture.events[${index}].atMs`,
    );
    invariant(
      atMs >= previousAtMs,
      "terminal capture event timestamps must be monotonic",
    );
    invariant(
      index > 0 || atMs === 0,
      "the first terminal capture event must start at zero",
    );
    previousAtMs = atMs;
    totalBytes += decodeBase64(
      event.data,
      `terminalCapture.events[${index}].data`,
    ).length;
    invariant(
      totalBytes <= maxBytes,
      "terminal capture exceeds the 4 MiB byte bound",
    );
  }
  exactKeys(
    value.completion,
  ...
  integer(
    value.completion.eventCount,
    1,
    100_000,
    "terminalCapture.completion.eventCount",
  );
  ...
  invariant(
    JSON.stringify(value.authority.nonAuthorities) ===
      JSON.stringify(TERMINAL_CAPTURE_NON_AUTHORITIES),
...
export function validateRenditionSet(output, helpers) {
  ...
  invariant(
    value.authority.classification === "capture-routing-metadata" &&
      ...
      value.authority.grants.length === 0 &&
      JSON.stringify(value.authority.nonAuthorities) ===
        JSON.stringify(RENDITION_SET_NON_AUTHORITIES),
    "rendition set authority boundary is invalid",
  );
  invariant(
    Array.isArray(value.renditions) && value.renditions.length === 2,
    "rendition set must contain exactly two captures",
  );
  const expected = [
    {
      id: "1080p",
      role: "primary",
      transcript: "complete-transcript.txt",
      projection: "public-projection.json",
      scene: "scene.json",
      terminalCapture: "terminal-capture.json",
      width: 1920,
      height: 1080,
    },
    {
      id: "720p",
      role: "responsive",
      transcript: "complete-transcript-720p.txt",
      projection: "public-projection-720p.json",
      scene: "scene-720p.json",
      terminalCapture: "terminal-capture-720p.json",
      width: 1280,
      height: 720,
    },
  ];
  const normalized = value.renditions.map((entry, index) => {
    ...
    for (const key of [
      ...
      invariant(
        ...
        `${label}.${key} is not the exact native rendition contract`,
      );
    }
    const transcriptBytes = readRegular(
      path.join(output, entry.transcript),
      `${label} transcript`,
      4 * 1024 * 1024,
    );
    ...
    invariant(
      ...
      `${label} transcript must not be empty`,
    );
    const lines = transcript.endsWith("\n")
      ? transcript.slice(0, -1).split("\n")
      : transcript.split("\n");
    const scene = validateScene(
      readJson(path.join(output, entry.scene), `${label} scene`),
    );
    invariant(
      scene.width === declaration.width && scene.height === declaration.height,
      `${label} scene dimensions are not native`,
    );
    const projection = validateProjection(
      readJson(path.join(output, entry.projection), `${label} projection`),
      scene,
      lines.length,
    );
    const captureBytes = readRegular(
      path.join(output, entry.terminalCapture),
      `${label} terminal capture`,
      maxBytes,
    );
    const capture = validateTerminalCapture(
      JSON.parse(decodeUtf8(captureBytes, `${label} terminal capture`)),
      scene,
      helpers,
    );
    invariant(
      entry.captureRoot === sha256(captureBytes),
      `${label}.captureRoot mismatch`,
    );
    return {
      ...entry,
      files: Object.fromEntries(
        ["transcript", "projection", "scene", "terminalCapture"].map((key) => [
          key,
          entry[key],
        ]),
      ),
  ...
  invariant(
    (normalized[0].scene.durationClass ?? "standard") ===
      (normalized[1].scene.durationClass ?? "standard"),
    "native rendition duration classes must match",
  );
  invariant(
    JSON.stringify(normalized[0].capture.dimensions) !==
      JSON.stringify(normalized[1].capture.dimensions),
    "native rendition PTY dimensions must be distinct",
  );
  invariant(
    normalized[0].projection.evidenceClass ===
      normalized[1].projection.evidenceClass &&
      normalized[0].projection.claimBoundary ===
        normalized[1].projection.claimBoundary,
    "native rendition evidence boundaries must match",
  );
  return {
    schema: value.schema,
    renditions: normalized,
