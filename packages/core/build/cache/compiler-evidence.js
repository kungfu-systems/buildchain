...
function assertCommandSucceeded(result, label) {
  ...
  if (result?.status !== 0) {
    ...
    throw new Error(
      `${label} exited ${result?.status ?? "without status"}${detail ? `: ${detail}` : ""}`,
    );
  }
}
...
function sumCounters(value) {
  ...
  return Object.values(value).reduce(
    (total, entry) => total + sumCounters(entry),
    0,
  );
}
...
export function prepareCompilerCacheEvidence({
  ...
  const required =
    cleanText(env.BUILDCHAIN_COMPILER_CACHE_REQUIRED).toLowerCase() === "true";
  if (provider !== "sccache") {
    if (required) {
      throw new Error(
        `required compiler cache provider must be sccache, received ${provider || "empty"}`,
      );
    }
    return undefined;
  }
  ...
  const bindings = Object.fromEntries(
    [
      ...
      [
        "cacheProfileRoot",
        optionalDigest(
          env.SHIFU_CACHE_PROFILE_DIGEST,
  ...
  if (!bindings.sourceCommit) {
    throw new Error(
      "BUILDCHAIN_SOURCE_SHA is required for compiler-cache preparation evidence",
    );
  }
...
export function verifyCompilerCacheActivity({
  ...
  if (required && cacheableRequests < 1) {
    throw new Error("required sccache observed no cacheable compiler requests");
  }
  return {
    ...stats,
    cacheableRequests,
